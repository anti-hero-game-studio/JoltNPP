...
#include "DefaultMovementSet/Modes/JoltSmoothWalkingMode.h"
#include "DefaultMovementSet/Modes/JoltSmoothWalkingState.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltSmoothWalkingMode)

void UJoltSmoothWalkingMode::SimulationTick_Implementation(const FJoltSimulationTickParams& Params, FJoltMoverTickEndData& OutputState)
{
	...
	if (const FJoltSmoothWalkingState* InSpringState = Params.StartState.SyncState.Collection.FindDataByType<FJoltSmoothWalkingState>())
	{
		FJoltSmoothWalkingState& OutputSpringState = OutputState.SyncState.Collection.FindOrAddMutableDataByType<FJoltSmoothWalkingState>();
		OutputSpringState = *InSpringState;
	}
}

void UJoltSmoothWalkingMode::GenerateWalkMove_Implementation(FJoltMoverTickStartData& StartState, float DeltaSeconds, const FVector& DesiredVelocity,
	const FQuat& DesiredFacing, const FQuat& CurrentFacing, FVector& InOutAngularVelocityDegrees, FVector& InOutVelocity) 
{
	...
	// Find or add a FJoltSmoothWalkingState to the SyncState
	bool bSmoothWalkingStateAdded = false;
	FJoltSmoothWalkingState& SpringState = StartState.SyncState.Collection.FindOrAddMutableDataByType<FJoltSmoothWalkingState>(bSmoothWalkingStateAdded);
	...
	// carried between movement modes in FJoltUpdatedMotionState.
