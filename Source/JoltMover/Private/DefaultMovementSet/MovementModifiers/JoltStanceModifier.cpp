...
#include "DefaultMovementSet/MovementModifiers/JoltStanceModifier.h"
...
#include "DefaultMovementSet/Settings/JoltStanceSettings.h"
#include "Engine/Blueprint.h"
#include "GameFramework/Pawn.h"
#include "MoveLibrary/JoltMovementUtils.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltStanceModifier)
...
bool FStanceModifier::HasGameplayTag(FGameplayTag TagToFind, bool bExactMatch) const
{
	...
	if (bExactMatch)
	{
		return TagToFind.MatchesTagExact(JoltMover_IsCrouching);
	}

	return TagToFind.MatchesTag(JoltMover_IsCrouching);
}

void FStanceModifier::OnStart(UJoltMoverComponent* MoverComp, const FJoltMoverTimeStep& TimeStep, const FJoltMoverSyncState& SyncState, const FJoltMoverAuxStateContext& AuxState)
{
	if (UJoltStanceSettings* StanceSettings = MoverComp->FindSharedSettings_Mutable<UJoltStanceSettings>())
	{
		if (const UCapsuleComponent* CapsuleComponent = Cast<UCapsuleComponent>(MoverComp->GetUpdatedComponent()))
		{
			...
			case EStanceMode::Prone:
				UE_LOG(LogJoltMover, Warning, TEXT("Stance got into prone stance - That stance is not currently implemented."));
...
void FStanceModifier::OnEnd(UJoltMoverComponent* MoverComp, const FJoltMoverTimeStep& TimeStep, const FJoltMoverSyncState& SyncState, const FJoltMoverAuxStateContext& AuxState)
{
	...
	{
		if (const UCapsuleComponent* OriginalCapsule = UJoltMovementUtils::GetOriginalComponentType<UCapsuleComponent>(MoverComp->GetOwner()))
		{
...
void FStanceModifier::OnPreMovement(UJoltMoverComponent* MoverComp, const FJoltMoverTimeStep& TimeStep)
{
...
void FStanceModifier::OnPostMovement(UJoltMoverComponent* MoverComp, const FJoltMoverTimeStep& TimeStep, const FJoltMoverSyncState& SyncState, const FJoltMoverAuxStateContext& AuxState)
{
	FJoltMovementModifierBase::OnPostMovement(MoverComp, TimeStep, SyncState, AuxState);
}

FJoltMovementModifierBase* FStanceModifier::Clone() const
{
...
bool FStanceModifier::CanExpand(const UCharacterJoltMoverComponent* MoverComp) const
{
	...
	UJoltMovementUtils::InitCollisionParams(UpdatedCompAsPrimitive, CapsuleParams, ResponseParam);

	const FJoltUpdatedMotionState* SyncState = MoverComp->GetSyncState().Collection.FindDataByType<FJoltUpdatedMotionState>();
	...
	{
		// Expand in place
		bEncroached = UJoltMovementUtils::OverlapTest(UpdatedComponent, UpdatedCompAsPrimitive, PawnLocation, PawnRot, CollisionChannel, StandingCapsuleShape, MoverComp->GetOwner());
	}
	else
	{
		// Expand while keeping base location the same.
		FVector StandingLocation = PawnLocation + (HalfHeightDifference + .01f) * MoverComp->GetUpDirection();
		bEncroached = UJoltMovementUtils::OverlapTest(UpdatedComponent, UpdatedCompAsPrimitive, StandingLocation, PawnRot, CollisionChannel, StandingCapsuleShape, MoverComp->GetOwner());
	}

	return !bEncroached;
}

bool FStanceModifier::ShouldExpandingMaintainBase(const UCharacterJoltMoverComponent* MoverComp) const
{
...
void FStanceModifier::AdjustCapsule(UJoltMoverComponent* MoverComp, float OldHalfHeight, float NewHalfHeight, float NewEyeHeight)
{
	...
	{
		TSharedPtr<FJoltTeleportEffect> TeleportEffect = MakeShared<FJoltTeleportEffect>();
...
void FStanceModifier::ApplyMovementSettings(UJoltMoverComponent* MoverComp)
{
	switch (ActiveStance)
	{
	default:
	case EStanceMode::Crouch:
		if (UJoltStanceSettings* StanceSettings = MoverComp->FindSharedSettings_Mutable<UJoltStanceSettings>())
		{
			...
			if (UJoltCommonLegacyMovementSettings* MovementSettings = MoverComp->FindSharedSettings_Mutable<UJoltCommonLegacyMovementSettings>())
			{
	...
	case EStanceMode::Prone:
		UE_LOG(LogJoltMover, Warning, TEXT("Stance got into prone stance - That mode is not currently implemented fully."));
		break;
	}
}

void FStanceModifier::RevertMovementSettings(UJoltMoverComponent* MoverComp)
{
	if (const UJoltMoverComponent* CDOMoverComp = UJoltMovementUtils::GetOriginalComponentType<UJoltMoverComponent>(MoverComp->GetOwner()))
	{
		const UJoltCommonLegacyMovementSettings* OriginalMovementSettings = CDOMoverComp->FindSharedSettings<UJoltCommonLegacyMovementSettings>();
		UJoltCommonLegacyMovementSettings* MovementSettings = MoverComp->FindSharedSettings_Mutable<UJoltCommonLegacyMovementSettings>();
