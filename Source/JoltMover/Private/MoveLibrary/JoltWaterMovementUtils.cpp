...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltWaterMovementUtils)

FJoltProposedMove UJoltWaterMovementUtils::ComputeControlledWaterMove(const FJoltWaterMoveParams& InParams)
{
	...
	OutMove.LinearVelocity = UJoltMovementUtils::ComputeCombinedVelocity(ComputeCombinedVelocityParams);
	...
	OutMove.AngularVelocityDegrees = UJoltMovementUtils::ComputeAngularVelocityDegrees(InParams.PriorOrientation, IntendedFacingDir.ToOrientationRotator(), InParams.DeltaSeconds, InParams.TurningRate);

	return OutMove;
}

void UJoltWaterMovementUtils::UpdateWaterSplineData(const FJoltUpdateWaterSplineDataParams& UpdateWaterSplineDataParams, FJoltWaterCheckResult& WaterCheckResult)
{
	if(WaterCheckResult.HitResult.Component.IsValid())
	{
		if (AWaterBody* WaterBody = Cast<AWaterBody>(WaterCheckResult.HitResult.Component->GetOwner()))
		{
			...
			{
				const TValueOrError<FWaterBodyQueryResult, EWaterBodyQueryError> QueryResult = WaterBody->GetWaterBodyComponent()->TryQueryWaterInfoClosestToWorldLocation(UpdateWaterSplineDataParams.PlayerLocation,
				...
				FJoltWaterFlowSplineData& WaterSplineData = WaterCheckResult.WaterSplineData;
		
				// Immersion depth
				WaterSplineData.ImmersionDepth = Query.GetImmersionDepth();
		
				WaterSplineData.WaterPlaneLocation = Query.GetWaterPlaneLocation();
				WaterSplineData.WaterPlaneNormal = Query.GetWaterPlaneNormal();
		
				// Water Depth
				WaterSplineData.WaterDepth = Query.GetWaterSurfaceDepth();
		
				// Water velocity
				WaterSplineData.RawWaterVelocity = Query.GetVelocity();
				...
				WaterSplineData.WaterSurfaceLocation = Query.GetWaterSurfaceLocation();
				...
				// Normal
				WaterSplineData.WaterSurfaceNormal = Query.GetWaterSurfaceNormal();
			}
		}
	}
}
