...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltAsyncMovementUtils)
...
/* static */
bool UJoltAsyncMovementUtils::TestDepenetratingMove(const FJoltMovingComponentSet& MovingComps, const FVector& StartLocation, const FVector& TargetLocation, const FQuat& StartRotation, const FQuat& TargetRotation, bool bShouldSweep, FHitResult& OutHit, FJoltMovementRecord& InOutMoveRecord)
{
	if (MovingComps.MoverComponent.Get() && MovingComps.MoverComponent.Get()->GetJoltPhysicsBodyComponent())
	{
		FJoltMoverCollisionParams CollisionParams(MovingComps.MoverComponent.Get()->GetJoltPhysicsBodyComponent());
...
/* static */
bool UJoltAsyncMovementUtils::TestDepenetratingMove(const FJoltMovingComponentSet& MovingComps, const FVector& StartLocation, const FVector& TargetLocation, const FQuat& StartRotation, const FQuat& TargetRotation, bool bShouldSweep, FJoltMoverCollisionParams& CollisionParams, FHitResult& OutHit, FJoltMovementRecord& InOutMoveRecord)
{
	...
	// Test full move first
	{
		...
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("TestDepenetratingMove: %s (role %i) Delta=%s DidMove=%i"),
	...
	if (OutHit.bStartPenetrating)
	{
		...
		if (FindMoveToResolveInitialPenetration_Internal(MovingComps, StartLocation, StartRotation, OutHit, CollisionParams, AdjustmentToResolve))
		{
			...
			InOutMoveRecord.Append(FJoltMovementSubstep(TestSafeDepenetrationSubstepName, AdjustmentToResolve, false));

			// Retry original move
			bDidMove |= TestMoveComponent_Internal(MovingComps, ResolvedStartLocation, TargetLocation, StartRotation, TargetRotation, bShouldSweep, CollisionParams, OutHit);

			UE_LOG(LogJoltMover, VeryVerbose, TEXT("TestDepenetratingMove retry: %s (role %i) Delta=%s DidMove=%i"),
				*GetNameSafe(UpdatedComponent->GetOwner()), UpdatedComponent->GetOwnerRole(), *(TargetLocation - ResolvedStartLocation).ToCompactString(), bDidMove);
	...
	if (bDidMove)
	{
		InOutMoveRecord.Append(FJoltMovementSubstep(TestSafeMoveSubstepName, CurrentLocation - ResolvedStartLocation, true));
	}
...
/* static */
float UJoltAsyncMovementUtils::TestSlidingMoveAlongHitSurface(const FJoltMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, FHitResult& InOutHit, FJoltMovementRecord& InOutMoveRecord)
{
...
/* static */ 
float UJoltAsyncMovementUtils::TestSlidingMoveAlongHitSurface(const FJoltMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, FJoltMoverCollisionParams& CollisionParams, FHitResult& InOutHit, FJoltMovementRecord& InOutMoveRecord)
{
...
/* static */
bool UJoltAsyncMovementUtils::FindMoveToResolveInitialPenetration_Internal(const FJoltMovingComponentSet& MovingComps, const FVector& StartLocation, const FQuat& StartRotation, const FHitResult& PenetratingHit, FJoltMoverCollisionParams& CollisionParams, FVector& OutAdjustmentDelta)
{
	if (ensureMsgf(PenetratingHit.bStartPenetrating, TEXT("Expected a hit that started penetrating. Will not attempt adjustment.")))
	{
		...
		FVector ProposedAdjustment = UJoltMovementUtils::ComputePenetrationAdjustment(PenetratingHit);
		ProposedAdjustment = UJoltPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), ProposedAdjustment);
		...
		if (!ProposedAdjustment.IsZero())
		{
			...
			bool bEncroached = UJoltMovementUtils::OverlapTest(UpdatedComponent, UpdatedPrimitive, PenetratingHit.TraceStart + ProposedAdjustment, StartRotation, UpdatedPrimitive->GetCollisionObjectType(), UpdatedPrimitive->GetCollisionShape(OverlapInflation), ActorOwner);
			...
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal: %s (role %i) Adjustment=%s DidMove=%i"),
			...
			if (!bMoved && SweepOutHit.bStartPenetrating)
			{
				...
				const FVector SecondMTD = UJoltMovementUtils::ComputePenetrationAdjustment(SweepOutHit);
				...
				if (SecondMTD != ProposedAdjustment && !CombinedMTD.IsZero())
				{
					...
					UE_LOG(LogJoltMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal combined: %s (role %i) CombinedAdjustment=%s DidMove=%i"),
			...
			if (!bMoved)
			{
				const UJoltMoverComponent* MoverComp = MovingComps.MoverComponent.Get();
				...
				const FVector FullMoveDelta = UJoltPlanarConstraintUtils::ConstrainDirectionToPlane(MoverComp->GetPlanarConstraint(), (PenetratingHit.TraceEnd - PenetratingHit.TraceStart));
				if (!FullMoveDelta.IsZero())
				{
					...
					UE_LOG(LogJoltMover, VeryVerbose, TEXT("FindMoveToResolveInitialPenetration_Internal multiple: %s (role %i) TargetLocationWithAdjustment=%s DidMove=%i"),
...
bool UJoltAsyncMovementUtils::TestMoveComponent_Internal(const FJoltMovingComponentSet& MovingComps, const FVector& StartLocation, const FVector& TargetLocation, const FQuat& StartRotation, const FQuat& TargetRotation, bool bShouldSweep, FJoltMoverCollisionParams& CollisionParams, FHitResult& OutHit)
{
	...
	{
		ConstrainedDelta = UJoltPlanarConstraintUtils::ConstrainDirectionToPlane(MovingComps.MoverComponent->GetPlanarConstraint(), ConstrainedDelta);
	}
	...
	{
		UJoltPhysicsWorldSubsystem* Subsystem = MyWorld->GetSubsystem<UJoltPhysicsWorldSubsystem>();
