...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltRollbackBlackboard)


uint32 UJoltRollbackBlackboard::BlackboardEntryBase::ComputeBufferSize(EJoltBlackboardSizingPolicy SizingPolicy, uint32 FixedBufferSize)
{
	uint32 BufferSize = 2;
	...
	switch (SizingPolicy)
	{
		case EJoltBlackboardSizingPolicy::FixedDeclaredSize:
...
UJoltRollbackBlackboard::BlackboardEntryBase::BlackboardEntryBase(const EntrySettings& InSettings, uint32 BufferSize)
	: Settings(InSettings)
...
void UJoltRollbackBlackboard::BlackboardEntryBase::RollBack(uint32 NewPendingFrame)
{
...
bool UJoltRollbackBlackboard::BlackboardEntryBase::CanReadEntryAt(const EntryTimeStamp& ReaderTimeStamp, EEntryIndexType IndexType) const
{
	...
	{
		case EJoltBlackboardPersistencePolicy::NextFrameOnly:
		{
		...
		case EJoltBlackboardPersistencePolicy::Forever:		// Always allow reading
			break;
...
void UJoltRollbackBlackboard::BeginSimulationFrame(const FJoltMoverTimeStep& PendingTimeStep)
{
...
void UJoltRollbackBlackboard::EndSimulationFrame()
{
...
void UJoltRollbackBlackboard::BeginRollback(const FJoltMoverTimeStep& NewBaseTimeStep)
{
	check(!bIsSimulationInProgress && !bIsRollbackInProgress);
	InRollbackThreadId = FPlatformTLS::GetCurrentThreadId();
	bIsRollbackInProgress = true;

	UE_LOG(LogJoltMover, Verbose, TEXT("Blackboard begin rollback. From Sim F %i / T %.3f -> F %i / T %.3f"), 
...
void UJoltRollbackBlackboard::EndRollback()
{
...
void UJoltRollbackBlackboard_InternalWrapper::BeginSimulationFrame(const FJoltMoverTimeStep& PendingTimeStep)
{
	Blackboard->BeginSimulationFrame(PendingTimeStep);
}

void UJoltRollbackBlackboard_InternalWrapper::EndSimulationFrame()
{
	Blackboard->EndSimulationFrame();
}

void UJoltRollbackBlackboard_InternalWrapper::BeginRollback(const FJoltMoverTimeStep& NewBaseTimeStep)
{
	Blackboard->BeginRollback(NewBaseTimeStep);
}

void UJoltRollbackBlackboard_InternalWrapper::EndRollback()
{
