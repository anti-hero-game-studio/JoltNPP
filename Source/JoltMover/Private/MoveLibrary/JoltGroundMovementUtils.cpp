...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltGroundMovementUtils)

FJoltProposedMove UJoltGroundMovementUtils::ComputeControlledGroundMove(const FJoltGroundMoveParams& InParams)
{
	FJoltProposedMove OutMove;

	const FVector MoveDirIntent = UJoltMovementUtils::ComputeDirectionIntent(InParams.MoveInput, InParams.MoveInputType, InParams.MaxSpeed);

	const FPlane MovementPlane(FVector::ZeroVector, InParams.UpDirection);
	FVector MoveDirIntentInMovementPlane = UJoltMovementUtils::ConstrainToPlane(MoveDirIntent, MovementPlane, true);

	const FPlane GroundSurfacePlane(FVector::ZeroVector, InParams.GroundNormal);
	OutMove.DirectionIntent = UJoltMovementUtils::ConstrainToPlane(MoveDirIntentInMovementPlane, GroundSurfacePlane, true);
	...
	const FVector Velocity = UJoltMovementUtils::ComputeVelocity(ComputeVelocityParams);
	OutMove.LinearVelocity = UJoltMovementUtils::ConstrainToPlane(Velocity, GroundSurfacePlane, true);
...
bool UJoltGroundMovementUtils::TryMoveToStepUp(const FJoltMovingComponentSet& MovingComps, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, float FloorSweepDistance, const FVector& MoveDelta, const FHitResult& MoveHitResult, const FJoltFloorCheckResult& CurrentFloor, bool bIsFalling, FJoltOptionalFloorCheckResult* OutFloorTestResult, FJoltMovementRecord& MoveRecord)
{
	...
	TArray<FJoltMovementSubstep> QueuedSubsteps;	// keeping track of substeps before committing, because some moves can be backed out
	...
	{
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("Not stepping up due to top of capsule hitting something"));
		return false;
	}
	...
	if (GravDir.IsZero())
	{
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("Not stepping up because there's no gravity"));
		return false;
	}
	...
	if (CurrentFloor.IsWalkableFloor())
	{
		...
		const bool bHitVerticalFace = !UJoltFloorQueryUtils::IsWithinEdgeTolerance(MoveHitResult.Location, MoveHitResult.ImpactPoint, PawnRadius, UpDirection);
	...
	{
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("Not stepping up because the impact is below us"));
		return false;
	}
	...
	const bool bDidStepUp = UJoltMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, UpAdjustment, PawnRotation, true, MOVECOMP_NoFlags, &SweepUpHit, ETeleportType::None);

	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TryMoveToStepUp Up: %s (role %i) UpAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *UpAdjustment.ToCompactString(), bDidStepUp);

	if (SweepUpHit.bStartPenetrating)
	{
		// Undo movement
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reverting step-up attempt because we started in a penetrating state"));
		ScopedStepUpMovement.RevertMove();
		return false;
	}

	// Cache upwards substep
	QueuedSubsteps.Add(FJoltMovementSubstep(StepUpSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, false));
	LastComponentLocation = CapsuleComponent->GetComponentLocation();

	// step fwd
	FHitResult StepFwdHit(1.f);
	const bool bDidStepFwd = UJoltMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, MoveDelta, PawnRotation, true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TryMoveToStepUp Fwd: %s (role %i) MoveDelta=%s DidMove=%i"),
	...
	if (StepFwdHit.bBlockingHit)
	{
		...
		{
			// Undo movement
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we started in a penetrating state"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			FJoltMoverOnImpactParams ImpactParams(NAME_None, SweepUpHit, MoveDelta);
		...
		if (MoverComp)
		{
			FJoltMoverOnImpactParams ImpactParams(NAME_None, StepFwdHit, MoveDelta);
			MoverComp->HandleImpact(ImpactParams);
		}
		
		if (bIsFalling)
		{
			QueuedSubsteps.Add( FJoltMovementSubstep(StepFwdSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, true) );

			// Commit queued substeps to movement record
			for (FJoltMovementSubstep Substep : QueuedSubsteps)
			{
		...
		QueuedSubsteps.Add( FJoltMovementSubstep(SlideSubstepName, CapsuleComponent->GetComponentLocation()-LastComponentLocation, true) );
		LastComponentLocation = CapsuleComponent->GetComponentLocation();
		MoveRecord.UnlockRelevancy();

		if (bIsFalling)
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because we could not adjust without falling"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reverting step-fwd attempt during step-up, because no movement differences occurred"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
	}
	...
	const bool bDidStepDown = UJoltMovementUtils::TryMoveUpdatedComponent_Internal(MovingComps, StepDownAdjustment, CapsuleComponent->GetComponentQuat(), true, MOVECOMP_NoFlags, &StepFwdHit, ETeleportType::None);

	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TryMoveToStepUp Down: %s (role %i) StepDownAdjustment=%s DidMove=%i"),
	...
	if (StepFwdHit.bStartPenetrating)
	{
		UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reverting step-down attempt during step-up/step-fwd, because we started in a penetrating state"));
		ScopedStepUpMovement.RevertMove();
		return false;
	}

	FJoltOptionalFloorCheckResult StepDownResult;
	if (StepFwdHit.IsValidBlockingHit())
	{
		...
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, because it made us travel too high (too high Height %.3f) up from floor base %f to %f"), DeltaDot, PawnInitialFloorBaseDot, StepFwdHit.ImpactPoint.Z);
			ScopedStepUpMovement.RevertMove();
			return false;
		}

		// Reject unwalkable surface normals here.
		if (!UJoltFloorQueryUtils::IsHitSurfaceWalkable(StepFwdHit, UpDirection, MaxWalkSlopeCosine))
		{
			// Reject if normal opposes movement direction
			const bool bNormalTowardsMe = (MoveDelta | StepFwdHit.ImpactNormal) < 0.f;
			if (bNormalTowardsMe)
			{
				UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s opposed to movement"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
			...
			{
				UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to unwalkable normal %s above old position)"), *StepFwdHit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		}
		...
		if (!UJoltFloorQueryUtils::IsWithinEdgeTolerance(StepFwdHit.Location, StepFwdHit.ImpactPoint, PawnRadius, UpDirection))
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being outside edge tolerance"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being up onto surface with !CanStepUpOnHitSurface")); 
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (OutFloorTestResult != NULL)
		{
			...
			UJoltFloorQueryUtils::FindFloor(MovingComps, FloorSweepDistance, MaxWalkSlopeCosine, bUseFlatBaseForFloorChecks, CapsuleComponent->GetComponentLocation(), StepDownResult.FloorTestResult);
			...
			if (StepFwdHit.Location.Dot(UpDirection) > OldLocationDot)
			{
				...
				{
					UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to it being an unperchable step")); 
					ScopedStepUpMovement.RevertMove();
					return false;
				}
			}
	...
	QueuedSubsteps.Add(FJoltMovementSubstep(StepDownSubstepName, CapsuleComponent->GetComponentLocation() - LastComponentLocation, false));
	LastComponentLocation = CapsuleComponent->GetComponentLocation();
...
static bool TryMoveToAdjustHeightAboveFloorInternal(const FJoltMovingComponentSet& MovingComps, FJoltFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, bool bPushAwayOnly, FJoltMovementRecord& MoveRecord)
{
	...
	if (OldFloorDist < UE::FloorQueryUtility::MIN_FLOOR_DIST || OldFloorDist > UE::FloorQueryUtility::MAX_FLOOR_DIST)
	{
		...
		UJoltMovementUtils::TrySafeMoveUpdatedComponent(MovingComps,
		...
		else
		{
			...
			if (UJoltFloorQueryUtils::IsHitSurfaceWalkable(AdjustHit, UpDirection, MaxWalkSlopeCosine))
			{
...
bool UJoltGroundMovementUtils::TryMoveToAdjustHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, FJoltFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FJoltMovementRecord& MoveRecord)
{
...
bool UJoltGroundMovementUtils::TryMoveToKeepMinHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, FJoltFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FJoltMovementRecord& MoveRecord)
{
	return TryMoveToAdjustHeightAboveFloorInternal(MovingComps, CurrentFloor, MaxWalkSlopeCosine, /*bPushAwayOnly*/true, MoveRecord);
}


float UJoltGroundMovementUtils::TryWalkToSlideAlongSurface(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, FHitResult& Hit, bool bHandleImpact, FJoltMovementRecord& MoveRecord, float MaxWalkSlopeCosine, float MaxStepHeight)
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
	...
	if (UJoltMoverComponent* MoverComponent = MovingComps.MoverComponent.Get())
	{
		...
		if (NormalDot > 0.f && !UJoltFloorQueryUtils::IsHitSurfaceWalkable(Hit, UpDirection, MaxWalkSlopeCosine))
		{
		...
		FVector SlideDelta = UJoltMovementUtils::ComputeSlideDelta(MovingComps, Delta, PctOfDeltaToMove, SafeWalkNormal, Hit);
		...
		if (SlideDelta.Dot(Delta) > 0.f)
		{
			...
			if (Hit.IsValidBlockingHit())
			{
				...
				SlideDelta = UJoltMovementUtils::ComputeTwoWallAdjustedDelta(MovingComps, SlideDelta, Hit, OldSafeHitNormal);
				const float SlideDeltaDot = SlideDelta.Dot(UpDirection);
				const float HitNormalDot = Hit.Normal.Dot(UpDirection);
				if (SlideDeltaDot > 0.f && UJoltFloorQueryUtils::IsHitSurfaceWalkable(Hit, UpDirection, MaxWalkSlopeCosine) && HitNormalDot > UE_KINDA_SMALL_NUMBER)
				{
...
FVector UJoltGroundMovementUtils::ComputeDeflectedMoveOntoRamp(const FVector& OrigMoveDelta, const FVector& UpDirection, const FHitResult& RampHitResult, float MaxWalkSlopeCosine, const bool bHitFromLineTrace)
{
	...
	if (FloorNormalDot < (1.f - UE_KINDA_SMALL_NUMBER) && FloorNormalDot > UE_KINDA_SMALL_NUMBER && 
		...
		UJoltFloorQueryUtils::IsHitSurfaceWalkable(RampHitResult, UpDirection, MaxWalkSlopeCosine) && !bHitFromLineTrace)
	{
		// Compute a vector that moves parallel to the surface, by projecting the horizontal movement direction onto the ramp.
		const FPlane RampSurfacePlane(FVector::ZeroVector, FloorNormal);
		return UJoltMovementUtils::ConstrainToPlane(OrigMoveDelta, RampSurfacePlane, true);
	}

	return OrigMoveDelta;
}

bool UJoltGroundMovementUtils::CanStepUpOnHitSurface(const FHitResult& Hit)
{
	if (!Hit.IsValidBlockingHit())
	{
		return false;
...
float UJoltGroundMovementUtils::TestGroundedMoveAlongHitSurface(const FJoltMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, bool bHandleImpact, float MaxStepHeight, float MaxWalkSlopeCosine, FHitResult& InOutHit, FJoltMovementRecord& InOutMoveRecord)
{
	...
	if (UJoltMoverComponent* MoverComponent = MovingComps.MoverComponent.Get())
	{
		...
		// We don't want to be pushed up an unwalkable surface.
		if (NormalDot > 0.f && !UJoltFloorQueryUtils::IsHitSurfaceWalkable(InOutHit, UpDirection, MaxWalkSlopeCosine))
		{
			SafeWalkNormal = FVector::VectorPlaneProject(SafeWalkNormal, -UpDirection).GetSafeNormal();
		}
		...
		FVector SlideDelta = UJoltMovementUtils::ComputeSlideDelta(MovingComps, OriginalMoveDelta, PctOfOrigDeltaToSlide, SafeWalkNormal, InOutHit);
		...
		if (SlideDelta.Dot(OriginalMoveDelta) > 0.f)
		{
			...
			if (InOutHit.IsValidBlockingHit())
			{
				...
				// Compute new slide normal when hitting multiple surfaces.
				SlideDelta = UJoltMovementUtils::ComputeTwoWallAdjustedDelta(MovingComps, SlideDelta, InOutHit, OldSafeHitNormal);
				const float SlideDeltaDot = SlideDelta.Dot(UpDirection);
				const float HitNormalDot = InOutHit.Normal.Dot(UpDirection);
				if (SlideDeltaDot > 0.f && UJoltFloorQueryUtils::IsHitSurfaceWalkable(InOutHit, UpDirection, MaxWalkSlopeCosine) && HitNormalDot > UE_KINDA_SMALL_NUMBER)
				{
					// Maintain horizontal velocity
				...
				{
					// Perform second move
					UJoltAsyncMovementUtils::TestDepenetratingMove(MovingComps, LocationInProgress, LocationInProgress + SlideDelta, TargetRotation, TargetRotation, /*bShouldSweep*/ true, InOutHit, InOutMoveRecord);
...
static FVector TestMoveToAdjustToFloorInternal(const FJoltMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, bool bPushAwayOnly, FJoltFloorCheckResult& InOutCurrentFloor, FJoltMovementRecord& InOutMoveRecord)
{
	// If we have a floor check that hasn't hit anything, don't adjust height.
	...
	if (OldFloorDist < UE::FloorQueryUtility::MIN_FLOOR_DIST || OldFloorDist > UE::FloorQueryUtility::MAX_FLOOR_DIST)
	{
		...
		UJoltAsyncMovementUtils::TestDepenetratingMove(MovingComps, Location, TargetAdjustedLocation, Rotation, Rotation, /*bShouldSweep*/ true, AdjustHit, InOutMoveRecord);
...
FVector UJoltGroundMovementUtils::TestMoveToAdjustToFloor(const FJoltMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, FJoltFloorCheckResult& InOutCurrentFloor, FJoltMovementRecord& InOutMoveRecord)
{
...
FVector UJoltGroundMovementUtils::TestMoveToKeepMinHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, UPARAM(ref) FJoltFloorCheckResult& InOutCurrentFloor, UPARAM(ref) FJoltMovementRecord& InOutMoveRecord)
{
...
/* static */
bool UJoltGroundMovementUtils::TestMoveToStepOver(const FJoltMovingComponentSet& MovingComps, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, float FloorSweepDistance, const FVector& MoveDelta, const FQuat& Rotation, const FHitResult& MoveHitResult, const FJoltFloorCheckResult& CurrentFloor, bool bIsFalling, FJoltOptionalFloorCheckResult* OutFloorTestResult, FVector& OutFinalLocation, FJoltMovementRecord& InOutMoveRecord)
{
	UCapsuleComponent* CapsuleComponent = Cast<UCapsuleComponent>(MovingComps.UpdatedPrimitive.Get());

	if (!UJoltGroundMovementUtils::CanStepUpOnHitSurface(MoveHitResult) || MaxStepHeight <= 0.f)
	{
		return false;
	}
	...
	FJoltMoverCollisionParams CollisionParams(MovingComps.UpdatedComponent.Get());
	// step up - treat as vertical wall
	FHitResult SweepUpHit(1.f);

	const FVector UpAdjustment = -GravDir * StepTravelUpHeight;

	const bool bDidStepUp = UJoltAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + UpAdjustment, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, SweepUpHit);

	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TestMoveToStepOver Up: %s (role %i) UpAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *UpAdjustment.ToCompactString(), bDidStepUp);
	...
	// Cache upwards substep
	QueuedSubsteps.Add(FJoltMovementSubstep(StepUpSubstepName, UpStepDelta, false));
	LocationInProgress += UpStepDelta;

	// step fwd
	FHitResult StepFwdHit(1.f);
	const bool bDidStepFwd = UJoltAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + MoveDelta, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, StepFwdHit);

	FVector FwdStepDelta = (StepFwdHit.TraceStart + ((StepFwdHit.TraceEnd - StepFwdHit.TraceStart) * StepFwdHit.Time)) - LocationInProgress;

	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TestMoveToStepOver Fwd: %s (role %i) MoveDelta=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *MoveDelta.ToCompactString(), bDidStepFwd);
	...
	if (StepFwdHit.bBlockingHit)
	{
		...
		// Cache forwards substep before the slide attempt
		QueuedSubsteps.Add(FJoltMovementSubstep(StepFwdSubstepName, FwdStepDelta, true));
		...
		QueuedSubsteps.Add(FJoltMovementSubstep(SlideSubstepName, FwdSlideDelta, true));
	...
	const bool bDidStepDown = UJoltAsyncMovementUtils::TestMoveComponent_Internal(MovingComps, LocationInProgress, LocationInProgress + StepDownAdjustment, Rotation, Rotation, /*bShouldSweep=*/ true, CollisionParams, StepFwdHit);


	UE_LOG(LogJoltMover, VeryVerbose, TEXT("TestMoveToStepOver Down: %s (role %i) StepDownAdjustment=%s DidMove=%i"),
		*GetNameSafe(CapsuleComponent->GetOwner()), CapsuleComponent->GetOwnerRole(), *StepDownAdjustment.ToCompactString(), bDidStepDown);
	...
	if (StepFwdHit.IsValidBlockingHit())
	{
		...
		// Don't step up onto invalid surfaces if traveling higher.
		if (DeltaDot > 0.f && !UJoltGroundMovementUtils::CanStepUpOnHitSurface(StepFwdHit))
		{
			UE_LOG(LogJoltMover, VeryVerbose, TEXT("Reject step-down attempt during step-up/step-fwd, due to being up onto surface with !CanStepUpOnHitSurface"));
			return false;
		}
		...
		if (OutFloorTestResult != NULL)
		{
			...
			UJoltFloorQueryUtils::FindFloor(MovingComps, FloorSweepDistance, MaxWalkSlopeCosine, bUseFlatBaseForFloorChecks, LocationInProgress, StepDownResult.FloorTestResult);
	...
	// Cache downwards substep
	QueuedSubsteps.Add(FJoltMovementSubstep(StepDownSubstepName, StepDownDelta, false));
