...
#include "GameplayDebuggerCategory_JoltMover.h"
...
namespace
{
	FAutoConsoleVariableRef CVars_GameplayDebuggerCategory_JoltMover[] = 
	{
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.MaxMoveIntentDrawLength"), 
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.OrientationDrawLength"),
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.ShowTrajectory"), 
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.ShowTrail"),
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.ShowCorrections"),
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.ShowStateArrows"),
		...
		FAutoConsoleVariableRef(TEXT("jolt.mover.debug.ShowInputArrows"),
...
FGameplayDebuggerCategory_JoltMover::FGameplayDebuggerCategory_JoltMover()
{
	SetDataPackReplication<FRepData>(&DataPack);
}


void FGameplayDebuggerCategory_JoltMover::CollectData(APlayerController* OwnerPC, AActor* DebugActor)
{
	APawn* MyPawn = Cast<APawn>(DebugActor);
	UJoltMoverComponent* MyMoverComponent = MyPawn ? Cast<UJoltMoverComponent>(MyPawn->GetComponentByClass(UJoltMoverComponent::StaticClass())) : nullptr;

	if (MyPawn)
	{
		// Get the debug component 
		UJoltMoverDebugComponent* MoverDebugComponent = Cast<UJoltMoverDebugComponent>(MyPawn->GetComponentByClass(UJoltMoverDebugComponent::StaticClass()));
		// Make it if we can't find it
		if (!MoverDebugComponent)
		{
			MoverDebugComponent = Cast<UJoltMoverDebugComponent>(MyPawn->AddComponentByClass(UJoltMoverDebugComponent::StaticClass(), false, FTransform::Identity, false));
	...
	if (MyMoverComponent)
	{
		...
		for (const TPair<FName, TObjectPtr<UJoltBaseMovementMode>>& ModeIter : MyMoverComponent->MovementModes)
		{
			UJoltBaseMovementMode* MappedMode = ModeIter.Value;
			...
			{
				for (UJoltBaseMovementModeTransition* Transition : MyMoverComponent->MovementModes[ModeIter.Key]->Transitions)
				{
		...
		for (UJoltBaseMovementModeTransition* Transition : MyMoverComponent->Transitions)
		{
			DataPack.ActiveTransitions.Add(FString::Printf(TEXT("%s (global)"), (Transition ? *Transition->GetClass()->GetName() : TEXT("null"))));
		}
		
		const FJoltMoverSyncState& SyncState = MyMoverComponent->GetSyncState();

		for (const TSharedPtr<FJoltLayeredMoveBase>& ActiveMove : SyncState.LayeredMoves.GetActiveMoves())
		{
		...
		const FJoltMoverInputCmdContext& LastInputCmd = MyMoverComponent->GetLastInputCmd();

		if (const FJoltCharacterDefaultInputs* DefaultInputs = LastInputCmd.Collection.FindDataByType<FJoltCharacterDefaultInputs>())
		{
...
void FGameplayDebuggerCategory_JoltMover::DrawData(APlayerController* OwnerPC, FGameplayDebuggerCanvasContext& CanvasContext)
{
...
TSharedRef<FGameplayDebuggerCategory> FGameplayDebuggerCategory_JoltMover::MakeInstance()
{
	return MakeShareable(new FGameplayDebuggerCategory_JoltMover());
}


void FGameplayDebuggerCategory_JoltMover::DrawOverheadInfo(AActor& DebugActor, FGameplayDebuggerCanvasContext& CanvasContext)
{
...
void FGameplayDebuggerCategory_JoltMover::DrawInWorldInfo(AActor& DebugActor, FGameplayDebuggerCanvasContext& CanvasContext)
{
	UWorld* MyWorld = CanvasContext.GetWorld();

	UJoltMoverComponent* MoverComp = Cast<UJoltMoverComponent>(DebugActor.GetComponentByClass(UJoltMoverComponent::StaticClass()));
	...
	if (FGameplayDebuggerCategoryTweakables::bShowStateArrowViz)
	{
		...
		if (MoverComp)
		{
			const FJoltMoverSyncState& LastState = MoverComp->GetSyncState();
			const FJoltUpdatedMotionState* MoverState = LastState.Collection.FindDataByType<FJoltUpdatedMotionState>();
...
void FGameplayDebuggerCategory_JoltMover::FRepData::Serialize(FArchive& Ar)
{
	Ar << PawnName;
	Ar << LocalRole;
	Ar << MovementModeName;
