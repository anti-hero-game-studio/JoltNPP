...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltMoverDebugComponent)

UJoltMoverDebugComponent::UJoltMoverDebugComponent()
{
...
void UJoltMoverDebugComponent::InitializeComponent()
{
...
void UJoltMoverDebugComponent::BeginPlay()
{
	Super::BeginPlay();
	
	if (UJoltMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UJoltMoverComponent>())
	{
		MoverComp->OnPostSimulationTick.AddDynamic(this, &UJoltMoverDebugComponent::OnMovementSimTick);
	}
}

void UJoltMoverDebugComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (UJoltMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UJoltMoverComponent>())
	{
...
void UJoltMoverDebugComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (bIsTrackingHistory)
	{
		UJoltMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UJoltMoverComponent>();
...
void UJoltMoverDebugComponent::SetHistoryTracking(float SecondsToTrack, float SamplesPerSecond)
{
...
TArray<FJoltTrajectorySampleInfo> UJoltMoverDebugComponent::GetPastTrajectory() const
{
	if (bIsTrackingHistory && !HistorySamples.IsEmpty())
	{
		TArray<FJoltTrajectorySampleInfo> HistoryAsArray;
	...
	return TArray<FJoltTrajectorySampleInfo>();
}

void UJoltMoverDebugComponent::InitHistoryTracking()
{
	if (UJoltMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UJoltMoverComponent>())
	{
		...
		if (bIsTrackingHistory)
		{
			...
			MoverComp->OnPostSimulationRollback.RemoveAll(this);
			MoverComp->OnPostSimulationRollback.AddDynamic(this, &UJoltMoverDebugComponent::OnHistoryTrackingRollback);
		}
		else
		{
		...
		MoverComp->OnPostSimulationRollback.AddDynamic(this, &UJoltMoverDebugComponent::OnMovementSimRollback);
	}
}

void UJoltMoverDebugComponent::UpdateHistoryTrackingForFrame(const FJoltMoverTimeStep& TimeStep, const FJoltMoverSyncState* SyncState, const FJoltMoverAuxStateContext* AuxState)
{
	const double CurrentSimTimeMs = TimeStep.BaseSimTimeMs;
	const double MaxTimeBetweenSamplesMs = (1000.0 / HistorySamplesPerSecond);
	const double MinSimTimeOfSampleMs = CurrentSimTimeMs - (HistoryTrackingSeconds * 1000.0);

	// Check if it's time to kick any samples out
	HistorySamples.RemoveAll([&](const FJoltTrajectorySampleInfo& Sample)
		{
	...
	FJoltTrajectorySampleInfo* MostRecentSample = HistorySamples.IsEmpty() ? nullptr : &HistorySamples.Last();

	if (!MostRecentSample || (CurrentSimTimeMs > MostRecentSample->SimTimeMs + MaxTimeBetweenSamplesMs))
	{
		const FJoltUpdatedMotionState* MoverState = SyncState->Collection.FindDataByType<FJoltUpdatedMotionState>();
		...
		FJoltTrajectorySampleInfo Sample;
...
void UJoltMoverDebugComponent::OnHistoryTrackingRollback(const FJoltMoverTimeStep& NewTimeStep, const FJoltMoverTimeStep& InvalidatedTimeStep)
{
	const double CurrentSimTimeMs = NewTimeStep.BaseSimTimeMs;
...
void UJoltMoverDebugComponent::DrawTrajectory()
{
	...
	if (UJoltMoverComponent* MoverComp = GetOwner()->FindComponentByClass<UJoltMoverComponent>())
	{
		...
		TArray<FJoltTrajectorySampleInfo> TrajectorySamples = MoverComp->GetPredictedTrajectory(PredictionParams);

		for (int32 i = 0; i < TrajectorySamples.Num() - 1; ++i)
		{
			const FJoltTrajectorySampleInfo& Sample = TrajectorySamples[i];
			const FJoltTrajectorySampleInfo& NextSample = TrajectorySamples[i + 1];
	...
	TArray<FJoltTrajectorySampleInfo> PastTrajectorySamples = GetPastTrajectory(); 

	for (int i = 0; i < PastTrajectorySamples.Num()-1; ++i)
	{
		const FJoltTrajectorySampleInfo& Sample = PastTrajectorySamples[i];
		const FJoltTrajectorySampleInfo& NextSample = PastTrajectorySamples[i + 1];
...
void UJoltMoverDebugComponent::OnMovementSimTick(const FJoltMoverTimeStep& TimeStep)
{
...
void UJoltMoverDebugComponent::OnMovementSimRollback(const FJoltMoverTimeStep& NewTimeStep, const FJoltMoverTimeStep& InvalidatedTimeStep)
{
...
void UJoltMoverDebugComponent::DrawCorrections()
{
	if (ClientLocations.Num() != CorrectedLocations.Num())
	{
		UE_LOG(LogJoltMover, Warning, TEXT("Correction arrays differed in size!"));
...
void UJoltMoverDebugComponent::DrawTrail()
{
	const float HalfHeight = GetOwner()->GetSimpleCollisionHalfHeight();
