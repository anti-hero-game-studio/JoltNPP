...
#include "JoltBlueprintDataExt.h"
#include "JoltMoverLog.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltBlueprintDataExt)


static constexpr uint8 NumValueMaps = 6;


bool FJoltMoverDictionaryData::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
...
void FJoltMoverDictionaryData::ToString(FAnsiStringBuilderBase& Out) const
{
	Super::ToString(Out);
...
void FJoltMoverDictionaryData::Interpolate(const FJoltMoverDataStructBase& From, const FJoltMoverDataStructBase& To, float Alpha)
{
	const FJoltMoverDictionaryData& FromTyped = *static_cast<const FJoltMoverDictionaryData*>(&From);
	const FJoltMoverDictionaryData& ToTyped = *static_cast<const FJoltMoverDictionaryData*>(&To);

	const FJoltMoverDictionaryData& PrimaryInfluence   = Alpha < 0.5f ? FromTyped : ToTyped;
	const FJoltMoverDictionaryData& SecondaryInfluence = Alpha < 0.5f ? ToTyped : FromTyped;
...
void FJoltMoverDictionaryData::Merge(const FJoltMoverDataStructBase& From)
{
	const FJoltMoverDictionaryData& FromTyped = *static_cast<const FJoltMoverDictionaryData*>(&From);
...
bool FJoltMoverDictionaryData::ShouldReconcile(const FJoltMoverDataStructBase& AuthorityState) const
{
