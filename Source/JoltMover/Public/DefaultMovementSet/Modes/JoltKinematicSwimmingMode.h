...
struct FJoltSwimmingControlSettings
{
	GENERATED_BODY()

	FJoltSwimmingControlSettings()
	{
	}
...
class UJoltKinematicSwimmingMode : public UJoltBaseMovementMode
{
...
public:
	...
	FJoltSwimmingControlSettings SurfaceSwimmingWaterControlSettings;

protected:
	UE_API virtual void OnRegistered(const FName ModeName) override; 
	UE_API virtual void OnUnregistered() override;

	UE_API virtual bool AttemptJump(const FJoltSimulationTickParams& Params, float UpwardsSpeed, FJoltMoverTickEndData& Output);

	UE_API void CaptureFinalState(USceneComponent* UpdatedComponent, const FJoltFloorCheckResult& FloorResult, const FJoltMovementRecord& Record, FJoltUpdatedMotionState& OutputSyncState) const;
