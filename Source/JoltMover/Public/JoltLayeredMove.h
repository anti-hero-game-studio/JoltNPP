...
#include "JoltLayeredMove.generated.h"
...
UENUM(BlueprintType)
enum class EJoltLayeredMoveFinishVelocityMode : uint8
{
...
struct FJoltLayeredMoveFinishVelocitySettings
{
	GENERATED_BODY()

	FJoltLayeredMoveFinishVelocitySettings()
		: SetVelocity(FVector::ZeroVector)
		, ClampVelocity(0.f)
		, FinishVelocityMode(EJoltLayeredMoveFinishVelocityMode::MaintainLastRootMotionVelocity)
	{}
	...
	// What mode we want to happen when a Layered Move ends, see @EJoltLayeredMoveFinishVelocityMode
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Mover)
	EJoltLayeredMoveFinishVelocityMode FinishVelocityMode;
};
...
struct FJoltLayeredMoveBase
{
	...
	UE_API FJoltLayeredMoveBase();
	virtual ~FJoltLayeredMoveBase() {}

	// Determines how this object's movement contribution should be mixed with others
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Mover)
	EJoltMoveMixMode MixMode;
	...
	UE_API void StartMove(const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	// Async version of the above
	UE_API void StartMove_Async(UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);

	// Called when this layered move starts.
	virtual void OnStart(const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard) {}
	// Async version of the above
	virtual void OnStart_Async(UJoltMoverBlackboard* SimBlackboard) {}

	// TODO: consider whether MoverComp should just be part of FJoltMoverTickStartData
	// Generate a movement that will be combined with other sources
	virtual bool GenerateMove(const FJoltMoverTickStartData& StartState, const FJoltMoverTimeStep& TimeStep, const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard, FJoltProposedMove& OutProposedMove) { return false; }
	// Async version of the above
	UE_API virtual bool GenerateMove_Async(const FJoltMoverTickStartData& StartState, const FJoltMoverTimeStep& TimeStep, UJoltMoverBlackboard* SimBlackboard, FJoltProposedMove& OutProposedMove);
	...
	UE_API void EndMove(const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	// Async version of the above
	UE_API void EndMove_Async(UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs);
	
	// Called when this layered move ends.
	virtual void OnEnd(const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs) {}
	// Async version of the above
	virtual void OnEnd_Async(UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs) {}

	// @return newly allocated copy of this FJoltLayeredMoveBase. Must be overridden by child classes
	UE_API virtual FJoltLayeredMoveBase* Clone() const;
...
template<>
struct TStructOpsTypeTraits< FJoltLayeredMoveBase > : public TStructOpsTypeTraitsBase2< FJoltLayeredMoveBase >
{
	enum
	{
...
USTRUCT(BlueprintType)
struct FJoltLayeredMoveGroup
{
	GENERATED_BODY()

	UE_API FJoltLayeredMoveGroup();
	...
	template <typename MoveType UE_REQUIRES(std::is_base_of_v<FJoltLayeredMoveBase, MoveType>)>
	bool HasMove() const
	{
	...
	UE_API TArray<TSharedPtr<FJoltLayeredMoveBase>> GenerateActiveMoves(const FJoltMoverTimeStep& TimeStep, const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard);
	UE_API TArray<TSharedPtr<FJoltLayeredMoveBase>> GenerateActiveMoves_Async(const FJoltMoverTimeStep& TimeStep, UJoltMoverBlackboard* SimBlackboard);
	...
	UE_API FJoltLayeredMoveGroup& operator=(const FJoltLayeredMoveGroup& Other);

	/** Comparison operator - needs matching LayeredMoves along with identical states in those structs */
	UE_API bool operator==(const FJoltLayeredMoveGroup& Other) const;

	/** Comparison operator */
	UE_API bool operator!=(const FJoltLayeredMoveGroup& Other) const;

	/** Checks only whether there are matching LayeredMoves, but NOT necessarily identical states of each move */
	UE_API bool HasSameContents(const FJoltLayeredMoveGroup& Other) const;
	...
	const TArray<TSharedPtr<FJoltLayeredMoveBase>>& GetActiveMoves() const { return ActiveLayeredMoves; }
	const TArray<TSharedPtr<FJoltLayeredMoveBase>>& GetQueuedMoves() const { return QueuedLayeredMoves; }
	...
	UE_API const FJoltLayeredMoveBase* FindActiveMove(const UScriptStruct* LayeredMoveStructType) const;
	...
	UE_API const FJoltLayeredMoveBase* FindQueuedMove(const UScriptStruct* LayeredMoveStructType) const;
...
protected:
	...
	UE_API void FlushMoveArrays(const UJoltMoverComponent* MoverComp, UJoltMoverBlackboard* SimBlackboard, double CurrentSimTimeMs, bool bIsAsync);

	// Helper function for gathering any residual velocity settings from layered moves that just ended
	UE_API void GatherResidualVelocitySettings(const TSharedPtr<FJoltLayeredMoveBase>& Move, bool& bResidualVelocityOverriden, bool& bClampVelocityOverriden);
	
	/** Helper function for serializing array of root motion sources */
	static UE_API void NetSerializeLayeredMovesArray(FArchive& Ar, TArray< TSharedPtr<FJoltLayeredMoveBase> >& LayeredMovesArray, uint8 MaxNumLayeredMovesToSerialize = MAX_uint8);

	/** Layered moves currently active in this group */
	TArray< TSharedPtr<FJoltLayeredMoveBase> > ActiveLayeredMoves;
...
template<>
struct TStructOpsTypeTraits<FJoltLayeredMoveGroup> : public TStructOpsTypeTraitsBase2<FJoltLayeredMoveGroup>
{
	enum
	{
		WithCopy = true,		// Necessary so that TSharedPtr<FJoltLayeredMoveBase> Data is copied around
		//WithNetSerializer = true,
