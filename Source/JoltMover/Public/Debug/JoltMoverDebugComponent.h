...
#include "JoltMoverDebugComponent.generated.h"
...
class UJoltMoverDebugComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	// Begin UActorComponent interface
	UE_API UJoltMoverDebugComponent();
	...
	UE_API void DrawCorrections();
	
	// Draw the trail behind the actor
	UE_API void DrawTrail();

	// Draw the current trajectory of the actor
	UE_API void DrawTrajectory();
    ...
    UE_API void SetHistoryTracking(float SecondsToTrack, float SamplesPerSecond);
	...
	UE_API TArray<FJoltTrajectorySampleInfo> GetPastTrajectory() const;
...
protected:
	...
	UE_API void InitHistoryTracking();
	UE_API void UpdateHistoryTrackingForFrame(const FJoltMoverTimeStep& TimeStep, const FJoltMoverSyncState* SyncState, const FJoltMoverAuxStateContext* AuxState);

	UFUNCTION()
	UE_API void OnHistoryTrackingRollback(const FJoltMoverTimeStep& NewTimeStep, const FJoltMoverTimeStep& InvalidatedTimeStep);

	bool bIsTrackingHistory = false;	// Master flag for whether trajectory history is being tracked
	TRingBuffer<FJoltTrajectorySampleInfo> HistorySamples;	// Running buffer of most recent trajectory
	// End Motion history tracking
	 
private:
	// Begin Trail simulation
	UFUNCTION()
	void OnMovementSimTick(const FJoltMoverTimeStep& TimeStep);

	UFUNCTION()
	void OnMovementSimRollback(const FJoltMoverTimeStep& NewTimeStep, const FJoltMoverTimeStep& InvalidatedTimeStep);
