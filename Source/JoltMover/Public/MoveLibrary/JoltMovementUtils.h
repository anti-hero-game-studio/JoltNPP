...
#include "JoltMovementUtils.generated.h"
...
{
	JOLTMOVER_API extern const double SMALL_MOVE_DISTANCE;
	JOLTMOVER_API extern const double VERTICAL_SLOPE_NORMAL_MAX_DOT;
}

/** Encapsulates detailed trajectory sample info, from a move that has already occurred or one projected into the future */
USTRUCT(BlueprintType)
struct FJoltTrajectorySampleInfo
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UJoltMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category=Mover)
	static UE_API FVector ComputeSlideDelta(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const float PctOfDeltaToMove, const FVector& Normal, const FHitResult& Hit);

	/** Returns an alternative move delta when we are in contact with 2 surfaces */
	static UE_API FVector ComputeTwoWallAdjustedDelta(const FJoltMovingComponentSet& MovingComps, const FVector& MoveDelta, const FHitResult& Hit, const FVector& OldHitNormal);

	/**
	 * Attempts to move a component along a surface. Returns the percent of time applied, with 0.0 meaning no movement occurred.
	 * Note: This function takes a movement record that collects moves applied to the actor see @FJoltMovementRecord
	 */
	UFUNCTION(BlueprintCallable, Category=Mover, meta=(DisplayName="Try Move To Slide Along Surface (Movement Record)"))
	static UE_API float TryMoveToSlideAlongSurface(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, UPARAM(Ref) FHitResult& Hit, bool bHandleImpact, UPARAM(Ref) FJoltMovementRecord& MoveRecord);
	...
	static UE_API float TryMoveToSlideAlongSurfaceNoMovementRecord(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, UPARAM(Ref) FHitResult& Hit, bool bHandleImpact);
	...
	static UE_API float TrySafeMoveAndSlideUpdatedComponent(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, UPARAM(Ref) FJoltMovementRecord& MoveRecord, bool bSlideAlongSurface = true);
	...
	static UE_API float TrySafeMoveAndSlideUpdatedComponentNoMovementRecord(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, bool bSlideAlongSurface = true);
	...
	static UE_API bool TrySafeMoveUpdatedComponent(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport, UPARAM(Ref) FJoltMovementRecord& MoveRecord);
	...
	static UE_API bool TrySafeMoveUpdatedComponentNoMovementRecord(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, UPARAM(Ref) FHitResult& OutHit, ETeleportType Teleport);
	...
	static UE_API bool TryMoveToResolvePenetration(const FJoltMovingComponentSet& MovingComps, EMoveComponentFlags MoveComponentFlags, const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotationQuat, FJoltMovementRecord& MoveRecord);
	...
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API FVector ComputeDirectionIntent(const FVector& MoveInput, EJoltMoveInputType MoveInputType, float MaxSpeed);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API bool FindTeleportSpot(const UJoltMoverComponent* MoverComp, UPARAM(Ref) FVector& TestLocation, FRotator TestRotation);

	/** Returns whether MoverComp's Actor would encroach at TestLocation on something that blocks it. */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API bool TestEncroachment(const UJoltMoverComponent* MoverComp, FVector TestLocation, FRotator TestRotation);
	 ...
	 */
	static UE_API bool TryMoveUpdatedComponent_Internal(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, const FQuat& NewRotation, bool bSweep, EMoveComponentFlags MoveComponentFlags, FHitResult* OutHit, ETeleportType Teleport);
...
template <class ComponentType>
const ComponentType* UJoltMovementUtils::GetOriginalComponentType(const AActor* MoverCompOwner)
{
