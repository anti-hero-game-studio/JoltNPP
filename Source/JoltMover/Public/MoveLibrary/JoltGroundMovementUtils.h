...
#include "JoltGroundMovementUtils.generated.h"
 ...
 */
UCLASS(MinimalAPI)
class UJoltGroundMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static UE_API FJoltProposedMove ComputeControlledGroundMove(const FJoltGroundMoveParams& InParams);
	...
	static UE_API bool TryMoveToStepUp(const FJoltMovingComponentSet& MovingComps, const FVector& GravDir, float MaxStepHeight, float MaxWalkSlopeCosine, bool bUseFlatBaseForFloorChecks, float FloorSweepDistance, const FVector& MoveDelta, const FHitResult& Hit, const FJoltFloorCheckResult& CurrentFloor, bool bIsFalling, FJoltOptionalFloorCheckResult* OutStepDownResult, FJoltMovementRecord& MoveRecord);
	...
	static UE_API bool TryMoveToAdjustHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, FJoltFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FJoltMovementRecord& MoveRecord);
	...
	static UE_API bool TryMoveToKeepMinHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, FJoltFloorCheckResult& CurrentFloor, float MaxWalkSlopeCosine, FJoltMovementRecord& MoveRecord);
     ...
     *  Note: This modifies the normal and calls UJoltMovementUtils::TryMoveToSlideAlongSurface
     */
    UFUNCTION(BlueprintCallable, Category=Mover)
    static UE_API float TryWalkToSlideAlongSurface(const FJoltMovingComponentSet& MovingComps, const FVector& Delta, float PctOfDeltaToMove, const FQuat Rotation, const FVector& Normal, UPARAM(ref) FHitResult& Hit, bool bHandleImpact, UPARAM(ref) FJoltMovementRecord& MoveRecord, float MaxWalkSlopeCosine, float MaxStepHeight);
...
public:	
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API float TestGroundedMoveAlongHitSurface(const FJoltMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, bool bHandleImpact, float MaxStepHeight, float MaxWalkSlopeCosine, UPARAM(ref) FHitResult& InOutHit, UPARAM(ref) FJoltMovementRecord& InOutMoveRecord);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API FVector TestMoveToAdjustToFloor(const FJoltMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, UPARAM(ref) FJoltFloorCheckResult& InOutCurrentFloor, UPARAM(ref) FJoltMovementRecord& InOutMoveRecord);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API FVector TestMoveToKeepMinHeightAboveFloor(const FJoltMovingComponentSet& MovingComps, const FVector& Location, const FQuat& Rotation, float MaxWalkSlopeCosine, UPARAM(ref) FJoltFloorCheckResult& InOutCurrentFloor, UPARAM(ref) FJoltMovementRecord& InOutMoveRecord);
