...
#include "JoltWaterMovementUtils.generated.h"
...
USTRUCT(BlueprintType)
struct FJoltWaterFlowSplineData
{
	GENERATED_BODY()
	
	FJoltWaterFlowSplineData()
	{
		Reset();
	}
...
struct FJoltWaterCheckResult
{
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Category = CharacterVolume)
	FJoltWaterFlowSplineData WaterSplineData; 
	
public:
	FJoltWaterCheckResult()
...
USTRUCT(BlueprintType)
struct FJoltUpdateWaterSplineDataParams
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UJoltWaterMovementUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static UE_API FJoltProposedMove ComputeControlledWaterMove(const FJoltWaterMoveParams& InParams);
	
	/** Updates the water spline data used in calculated swimming movement */
	UFUNCTION(BlueprintCallable, Category=Mover)
	static UE_API void UpdateWaterSplineData(const FJoltUpdateWaterSplineDataParams& UpdateWaterSplineDataParams, UPARAM(ref) FJoltWaterCheckResult& WaterCheckResult);
};
