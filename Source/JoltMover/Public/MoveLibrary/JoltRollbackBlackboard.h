...
#include "JoltRollbackBlackboard.generated.h"
 ...
 */
UENUM()
enum class EJoltBlackboardSizingPolicy : uint8
{
 ...
 */
UENUM()
enum class EJoltBlackboardPersistencePolicy : uint8
{
 ...
 */
UENUM()
enum class EJoltBlackboardRollbackPolicy : uint8
{
 ...
 * See @UJoltRollbackBlackboard_InternalWrapper for in-simulation use.
 */ 
UCLASS(MinimalAPI)
class UJoltRollbackBlackboard : public UObject
{
	GENERATED_BODY()
...
public:
	...
	{
		EJoltBlackboardSizingPolicy      SizingPolicy      = EJoltBlackboardSizingPolicy::FixedDeclaredSize;
		EJoltBlackboardPersistencePolicy PersistencePolicy = EJoltBlackboardPersistencePolicy::Forever;
		EJoltBlackboardRollbackPolicy    RollbackPolicy    = EJoltBlackboardRollbackPolicy::InvalidatedOnRollback;
...
private:
	...
	public:
		...
		UE_API static uint32 ComputeBufferSize(EJoltBlackboardSizingPolicy InSizingPolicy, uint32 FixedBufferSize = -1);
...
public:
	...
	bool CreateEntry(FName EntryName, const EntrySettings& InSettings)
	{
		...
		{
			UE_LOG(LogJoltMover, Warning, TEXT("Skipping attempt to create a new blackboard entry named %s since it already exists. Policy settings from the first entry will be retained."), *EntryName.ToString());
			return false;
		}
...
private:
	UE_API void BeginSimulationFrame(const FJoltMoverTimeStep& PendingTimeStep);
	UE_API void EndSimulationFrame();

	UE_API void BeginRollback(const FJoltMoverTimeStep& NewBaseTimeStep);	// NewBaseTimeStep represents the current time and the PENDING frame that's next to be resimulated
...
private:
	...
	friend class UJoltRollbackBlackboard_InternalWrapper;	// expose non-public API to this wrapper class
};
 ...
 */
UCLASS(MinimalAPI)
class UJoltRollbackBlackboard_InternalWrapper : public UObject
{
	GENERATED_BODY()

public:
	void Init(UJoltRollbackBlackboard& InBlackboard)
	{
		Blackboard = &InBlackboard;
	}

	template<typename EntryT>
	bool CreateEntry(FName EntryName, const UJoltRollbackBlackboard::EntrySettings& InSettings)
	{
	...
	UE_API void BeginRollback(const FJoltMoverTimeStep& NewBaseTimeStep);
	UE_API void EndRollback();

private: 
	UPROPERTY()
	TObjectPtr<UJoltRollbackBlackboard> Blackboard;
};
