...
#include "JoltConstrainedMoveUtils.generated.h"
 ...
 */
UCLASS(MinimalAPI)
class UJoltPlanarConstraintUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	/** Sets whether or not the constraint is enabled */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API void SetPlanarConstraintEnabled(UPARAM(ref) FJoltPlanarConstraint& Constraint, bool bEnabled);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API void SetPlanarConstraintNormal(UPARAM(ref) FJoltPlanarConstraint& Constraint, FVector PlaneNormal);

	/** Sets the origin of the plane that constrains movement, enforced if the plane constraint is enabled. */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API void SetPlaneConstraintOrigin(UPARAM(ref) FJoltPlanarConstraint& Constraint, FVector PlaneOrigin);

	// APPLICATION OF CONSTRAINT

	/** Constrains a direction to be on the plane, if enabled */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API FVector ConstrainDirectionToPlane(const FJoltPlanarConstraint& Constraint, FVector Direction, bool bMaintainMagnitude=false);

	/** Constrains a location to be on the plane, if enabled */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API FVector ConstrainLocationToPlane(const FJoltPlanarConstraint& Constraint, FVector Location);

	/** Constrains a normal to be on the plane, if enabled. Result will be re-normalized. */
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|PlanarConstraint")
	static UE_API FVector ConstrainNormalToPlane(const FJoltPlanarConstraint& Constraint, FVector Normal);

}; // UJoltPlanarConstraintUtils
