...
#include "JoltMoverTypes.generated.h"

#define UE_API JOLTMOVER_API

// Gameplay tags
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsOnGround);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsInAir);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsFalling);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsFlying);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsSwimming);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsCrouching);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_IsNavWalking);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_SkipAnimRootMotion);
JOLTMOVER_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(JoltMover_SkipVerticalAnimRootMotion);

/** List of Mover tick phases where different parts of work are being performed */
UENUM(BlueprintType)
enum class EJoltMoverTickPhase : uint8
{
...
UENUM(BlueprintType)
enum class EJoltMoverTickDependencyOrder : uint8
{
	Before = 0,
	After = 1,
};
...
UENUM(BlueprintType)
enum class EJoltMoverSmoothingMode : uint8
{
...
// Struct to hold params for when an impact happens. This contains all of the data for impacts including what gets passed to the FJoltMover_OnImpact delegate
USTRUCT(BlueprintType, meta = (DisplayName = "Impact Data"))
struct FJoltMoverOnImpactParams
{
	GENERATED_BODY()
	
	UE_API FJoltMoverOnImpactParams();
	
	UE_API FJoltMoverOnImpactParams(const FName& ModeName, const FHitResult& Hit, const FVector& Delta);
...
// NOTE: for simulation state data (sync/aux), derive from FJoltMoverStateData instead 
USTRUCT(BlueprintType)
struct FJoltMoverDataStructBase
{
	GENERATED_BODY()

	UE_API FJoltMoverDataStructBase();
	virtual ~FJoltMoverDataStructBase() {}

	/** return newly allocated copy of this FJoltMoverDataStructBase. MUST be overridden by derived types. */
	UE_API virtual FJoltMoverDataStructBase* Clone() const;
	...
	/** Gets the type info of this FJoltMoverDataStructBase. MUST be overridden by derived types. */
	 ...
	 */
	UE_API virtual bool ShouldReconcile(const FJoltMoverDataStructBase& AuthorityState) const;
	 ...
	 * Override for: All types that compose STATE data (sync or aux) and types that compose INPUT data for physics-based movement  
	 */
	UE_API virtual void Interpolate(const FJoltMoverDataStructBase& From, const FJoltMoverDataStructBase& To, float Pct);
	 ...
	 */
	UE_API virtual void Merge(const FJoltMoverDataStructBase& From);
...
template<>
struct TStructOpsTypeTraits< FJoltMoverDataStructBase > : public TStructOpsTypeTraitsBase2< FJoltMoverDataStructBase >
{
	enum
	{
...
// Contains a group of different FJoltMoverDataStructBase-derived data, and supports net serialization of them. Note that
...
struct FJoltMoverDataCollection
{
	...
	UE_API FJoltMoverDataCollection();
	...
	*/
	...
	UE_API FJoltMoverDataCollection& operator=(const FJoltMoverDataCollection& Other);
	...
	UE_API bool operator==(const FJoltMoverDataCollection& Other) const;

	/** Comparison operator */
	UE_API bool operator!=(const FJoltMoverDataCollection& Other) const;

	/** Checks if the collections are significantly different enough (piece-wise) to need reconciliation. NOT an equality check. */
	UE_API bool ShouldReconcile(const FJoltMoverDataCollection& Other) const;

	/** Make this collection a piece-wise interpolation between 2 collections */
	UE_API void Interpolate(const FJoltMoverDataCollection& From, const FJoltMoverDataCollection& To, float Pct);

	/** Merge a previous frame's collection with this collection */
	UE_API void Merge(const FJoltMoverDataCollection& From);
	...
	UE_API bool HasSameContents(const FJoltMoverDataCollection& Other) const;
	...
	UE_API TArray<TSharedPtr<FJoltMoverDataStructBase>>::TConstIterator GetCollectionDataIterator() const;
	...
	{
		if (FJoltMoverDataStructBase* FoundData = FindDataByType(T::StaticStruct()))
		{
	...
	{
		if (const FJoltMoverDataStructBase* FoundData = FindDataByType(T::StaticStruct()))
		{
	...
	UE_API void AddOrOverwriteData(const TSharedPtr<FJoltMoverDataStructBase> DataInstance);
	 ...
	 */
	UE_API void AddDataByCopy(const FJoltMoverDataStructBase* DataInstanceToCopy);


	const TArray< TSharedPtr<FJoltMoverDataStructBase> >& GetDataArray() const
	{
		return DataArray;
	}

	/** Find data of a specific type in the collection. */
	UE_API FJoltMoverDataStructBase* FindDataByType(const UScriptStruct* DataStructType) const;

	/** Find data of a specific type in the collection. If not found, a new default instance will be added. */
	UE_API FJoltMoverDataStructBase* FindOrAddDataByType(const UScriptStruct* DataStructType);
...
protected:
	UE_API FJoltMoverDataStructBase* AddDataByType(const UScriptStruct* DataStructType);
	static UE_API TSharedPtr<FJoltMoverDataStructBase> CreateDataByType(const UScriptStruct* DataStructType);
	
	/** Helper function for serializing array of data */
	static UE_API void NetSerializeDataArray(FArchive& Ar, UPackageMap* Map, TArray<TSharedPtr<FJoltMoverDataStructBase>>& DataArray);

	/** All data in this collection */
	TArray< TSharedPtr<FJoltMoverDataStructBase> > DataArray;


friend class UJoltMoverDataCollectionLibrary;
friend class UJoltMoverComponent;
};


template<>
struct TStructOpsTypeTraits<FJoltMoverDataCollection> : public TStructOpsTypeTraitsBase2<FJoltMoverDataCollection>
{
	enum
	{
...
struct FJoltMoverDataPersistence
{
	GENERATED_BODY()

	FJoltMoverDataPersistence() {}

	FJoltMoverDataPersistence(UScriptStruct* TypeToPersist, bool bShouldCopyBetweenFrames) 
	{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Mover, meta=(MetaStruct="/Script/JoltMover.MoverDataStructBase"))
...
struct FJoltMovementModeChangeRecord
{
	FName ModeName;
	FName PrevModeName;
	uint32 Frame;
...
UCLASS(MinimalAPI)
class UJoltMoverDataCollectionLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	* @param SourceAsRawBytes		The data struct instance to add by copy, which must be a FJoltMoverDataStructBase sub-type
	*/
	UFUNCTION(BlueprintCallable, CustomThunk, Category = "Jolt Mover|Data", meta = (CustomStructureParam = "SourceAsRawBytes", AllowAbstract = "false", DisplayName = "Add Data To Collection"))
	static UE_API void K2_AddDataToCollection(UPARAM(Ref) FJoltMoverDataCollection& Collection, UPARAM(DisplayName="Struct To Add") const int32& SourceAsRawBytes);
	 ...
	 * @param TargetAsRawBytes		The data struct instance to write to, which must be a FJoltMoverDataStructBase sub-type
	 */
	UFUNCTION(BlueprintCallable, CustomThunk, Category = "Jolt Mover|Data", meta = (CustomStructureParam = "TargetAsRawBytes", AllowAbstract = "false", DisplayName = "Get Data From Collection"))
	static UE_API void K2_GetDataFromCollection(bool& DidSucceed, UPARAM(Ref) const FJoltMoverDataCollection& Collection, UPARAM(DisplayName = "Out Struct") int32& TargetAsRawBytes);

	/**
	* Clears all data from a collection
	*/
	UFUNCTION(BlueprintCallable, Category = "Jolt Mover|Data", meta=(DisplayName = "Clear Data From Collection"))
	static UE_API void ClearDataFromCollection(UPARAM(Ref) FJoltMoverDataCollection& Collection);
