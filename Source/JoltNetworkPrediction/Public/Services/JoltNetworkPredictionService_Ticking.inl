...
class IJoltLocalTickService
{
public:

	virtual ~IJoltLocalTickService() = default;
	virtual void Tick(const FJoltNetSimTimeStep& Step, const FJoltServiceTimeStep& ServiceStep) = 0;
};

template<typename InModelDef>
class TJoltLocalTickServiceBase : public IJoltLocalTickService
{
public:
	...
	TJoltLocalTickServiceBase(TJoltModelDataStore<ModelDef>* InDataStore)
...
protected:
	...
	void Tick_Internal(const FJoltNetSimTimeStep& Step, const FJoltServiceTimeStep& ServiceStep)
	{
		...
		for (auto It : InstancesToTick)
		{
			...
			TJoltTickUtil<ModelDef>::DoTick(Instance, InputFrameData, OutputFrameData, Step, EndTime, GetTickContext<bIsResim>(Instance.NetRole));
		}
	}
...
template<typename InModelDef>
class TJoltLocalTickService : public TJoltLocalTickServiceBase<InModelDef>
{
public:
	TJoltLocalTickService(TJoltModelDataStore<InModelDef>* InDataStore) 
		: TJoltLocalTickServiceBase<InModelDef>(InDataStore)
	{
...
class IJoltRemoteIndependentTickService
{
public:

	virtual ~IJoltRemoteIndependentTickService() = default;
...
template<typename InModelDef>
class TJoltRemoteIndependentTickService : public IJoltRemoteIndependentTickService
{
public:
	using ModelDef = InModelDef;
	...
	TJoltRemoteIndependentTickService(TJoltModelDataStore<ModelDef>* InDataStore)
		: DataStore(InDataStore) { }
		...
		for (TConstSetBitIterator<> BitIt(InstanceBitArray); BitIt; ++BitIt)
		{
			...
			while (ServerRecvData.LastConsumedFrame < ServerRecvData.LastRecvFrame)
			{
				...
				// Tick
				{
					...
					TJoltTickUtil<ModelDef>::DoTick(InstanceData, InputFrameData, OutputFrameData, Step, CueTimeMS, EJoltSimulationTickContext::Authority);
