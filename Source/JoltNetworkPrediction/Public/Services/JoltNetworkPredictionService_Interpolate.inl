...
namespace NetworkPredictionCVars
{
	...
	JOLTNETSIM_DEVCVAR_SHIPCONST_INT(PrintSyncInterpolation, 0, "j.np.Interpolation.PrintSync", "Print Sync State buffer during interpolation");
	JOLTNETSIM_DEVCVAR_SHIPCONST_INT(DrawInterpolation, 0, "j.np.Interpolation.Draw", "Draw interpolation debug state in world");

	JOLTNETSIM_DEVCVAR_SHIPCONST_INT(DisableInterpolation, 0, "j.np.Interpolation.Disable", "Disables smooth interpolation and just Finalizes the last received frame");
}
...
// ------------------------------------------------------------------------------
class IJoltFixedInterpolateService
{
public:

	virtual ~IJoltFixedInterpolateService() = default;
	virtual void Reconcile(FJoltFixedTickState* TickState) = 0;
	virtual void FinalizeFrame(float DeltaTimeSeconds, const FJoltFixedTickState* TickState) = 0;
};

template<typename InModelDef>
class TJoltFixedInterpolateService : public IJoltFixedInterpolateService
{
public:
	...
	TJoltFixedInterpolateService(TJoltModelDataStore<ModelDef>* InDataStore)
	...
	void UnregisterInstance(FJoltNetworkPredictionID ID)
	{
		...
		const TJoltClientRecvData<ModelDef>& ClientRecvData = DataStore->ClientRecv.GetByIndexChecked(ClientRecvIdx);
		...
		if (InterpolationData.bTwoValidFrames == false)
		{
			...
			FJoltNetworkPredictionDriver<ModelDef>::SetHiddenForInterpolation(InstanceData.Info.Driver, false);
		}
	...
	void Reconcile(FJoltFixedTickState* TickState) final override
	{
		...
		for (TConstDualSetBitIterator<FDefaultBitArrayAllocator,FDefaultBitArrayAllocator> BitIt(ClientRecvBitMask, DataStore->ClientRecvBitMask); BitIt; ++BitIt)
		{
			...
			if (ToFrame != INDEX_NONE && LocalFrame - ToFrame > Frames.Buffer.Capacity())
			{
				UE_LOG(LogJoltNetworkPrediction,Error,TEXT("Local Frame %d , ToFrame %d, Capacity %d"),LocalFrame,ToFrame,Frames.Buffer.Capacity());
			}
			...
			//Edited By Kai Delta Serialization : Interpolated Save The Acked Frame
			uint32& AckedFrame = TickState->LocalAckedFrames.IDsToAckedFrames.FindOrAdd(ClientRecvData.ID);
			AckedFrame = LocalFrame;
			...
			if (InterpolationData.bTwoValidFrames == false)
			{
				...
				{
					TInstanceData<ModelDef>& InstanceData = DataStore->Instances.GetByIndexChecked(InterpolationData.InstanceIdx);
					FJoltNetworkPredictionDriver<ModelDef>::SetHiddenForInterpolation(InstanceData.Info.Driver, true);
				}
			}
	...
	void FinalizeFrame(float DeltaTimeSeconds, const FJoltFixedTickState* TickState) final override
	{
		const int32 FromFrame = TickState->Interpolation.ToFrame-1;
		const int32 ToFrame = TickState->Interpolation.ToFrame;
		...
		if ( NetworkPredictionCVars::DisableInterpolation() > 0)
		{
			for (TConstSetBitIterator<> BitIt(ClientRecvBitMask); BitIt; ++BitIt)
			{
				...
				FJoltNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, ClientRecvData.SyncState, ClientRecvData.AuxState);
				...
				FJoltNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, ClientRecvData.ServerFrame, ClientRecvData.SimTimeMS, 0);
			}
		...
		for (auto& It : Instances)
		{
			...
			{
				typename TJoltInstanceFrameState<ModelDef>::FFrame& FromFrameData = Frames.Buffer[FromFrame];
				typename TJoltInstanceFrameState<ModelDef>::FFrame& ToFrameData = Frames.Buffer[ToFrame];

				FJoltNetworkPredictionDriver<ModelDef>::Interpolate(SyncAuxType{FromFrameData.SyncState, FromFrameData.AuxState}, SyncAuxType{ToFrameData.SyncState, ToFrameData.AuxState}, 
				...
				FJoltNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, Instance.SyncState, Instance.AuxState);
				...
				FJoltNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, InterpolatedTimeMS, 0);
			}
		}
	}
...
private:
	...
	struct FInstance
	{
		FInstance(int32 InTraceID, int32 InInstanceIdx, int32 InFramesIdx)
			: TraceID(InTraceID), InstanceIdx(InInstanceIdx), FramesIdx(InFramesIdx) {} // fixme
		...
		// Last interpolated values. Stored here so that we can maintain FJoltNetworkPredictionStateView to them
	...
	static void CopyFrameData(int32 SourceFrame, int32 DestFrame, TJoltInstanceFrameState<ModelDef>& Frames, FInstance& Instance)
	{
		typename TJoltInstanceFrameState<ModelDef>::FFrame& SourceFrameData = Frames.Buffer[SourceFrame];
		typename TJoltInstanceFrameState<ModelDef>::FFrame& DestFrameData = Frames.Buffer[DestFrame];

		DestFrameData.SyncState = SourceFrameData.SyncState;
		DestFrameData.AuxState = SourceFrameData.AuxState;
		DestFrameData.InputCmd = SourceFrameData.InputCmd;
	}

	static void Interpolate(const int32 FromSourceFrame, const int32 ToSourceFrame, const int32 DestFrame, TJoltInstanceFrameState<ModelDef>& Frames, FInstance& Instance, const int32 FixedStepMS)
	{
		jnpEnsureSlow(FromSourceFrame < ToSourceFrame);
		jnpEnsureSlow(FromSourceFrame < DestFrame);
		jnpEnsureSlow(DestFrame < ToSourceFrame);

		typename TJoltInstanceFrameState<ModelDef>::FFrame& FromFrameData = Frames.Buffer[FromSourceFrame];
		typename TJoltInstanceFrameState<ModelDef>::FFrame& ToFrameData = Frames.Buffer[ToSourceFrame];
		typename TJoltInstanceFrameState<ModelDef>::FFrame& DestFrameData = Frames.Buffer[DestFrame];
		...
		// copy input command from ToFrame
		DestFrameData.InputCmd = ToFrameData.InputCmd;
	}
};
...
// ------------------------------------------------------------------------------
class IJoltIndependentInterpolateService
{
public:

	virtual ~IJoltIndependentInterpolateService() = default;
	virtual void Reconcile(const FJoltVariableTickState* TickState) = 0;
	virtual void FinalizeFrame(float DeltaTimeSeconds, const FJoltVariableTickState* TickState) = 0;
};


template<typename InModelDef>
class TJoltIndependentInterpolateService : public IJoltIndependentInterpolateService
{
public:
	...
	TJoltIndependentInterpolateService(TJoltModelDataStore<ModelDef>* InDataStore)
		: DataStore(InDataStore) { }
	...
	void FinalizeFrame(float DeltaTimeSeconds, const FJoltVariableTickState* TickState) final override
	{
		...
		for (auto& It : Instances)
		{
			...
			if (FromFrame != ToFrame)
			{
				...
				FJoltNetworkPredictionDriver<ModelDef>::Interpolate(SyncAuxType{FromFrameData.SyncState, FromFrameData.AuxState}, SyncAuxType{ToFromData.SyncState, ToFromData.AuxState}, 
					PCT, Instance.SyncState, Instance.AuxState);
				
				FJoltNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, Instance.SyncState, Instance.AuxState);

				InstanceData.Info.View->UpdateView(ToFrame, InterpolationTimeMS, nullptr, ToFromData.SyncState, ToFromData.AuxState);
				FJoltNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, InterpolationTimeMS, 0);
				
				if (NetworkPredictionCVars::DrawInterpolation())
				{
					FJoltNetworkPredictionDriver<ModelDef>::DrawDebugOutline(InstanceData.Info.Driver, FColor::Green, 0.f);

					const int32 DeltaFromHead = Instance.LastWrittenFrame - FromFrame;

					FJoltNetworkPredictionDriver<ModelDef>::DrawDebugText3D(InstanceData.Info.Driver, *FString::Printf(TEXT("[%d-%d] %d. %.2f"), FromFrame, ToFrame, DeltaFromHead, PCT), FColor::Black);
				}

				if (NetworkPredictionCVars::PrintSyncInterpolation())
				{
					UE_LOG(LogJoltNetworkPrediction, Display, TEXT(""));
					UE_LOG(LogJoltNetworkPrediction, Display, TEXT("Interpolation %s. Frames: %d - %d. InterpolationTimeMS: %d. PCT: %.2f"), ModelDef::GetName(), FromFrame, ToFrame, InterpolationTimeMS, PCT);

					UE_LOG(LogJoltNetworkPrediction, Display, TEXT("From: %d"), FromTimeMS);
					FJoltNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(FromFrameData.SyncState);

					UE_LOG(LogJoltNetworkPrediction, Display, TEXT("To: %d"), ToTimeMS);
					FJoltNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(ToFromData.SyncState);

					UE_LOG(LogJoltNetworkPrediction, Display, TEXT("Interpolated: "));
					FJoltNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(Instance.SyncState);
			...
			else
			{
				...
				{
					UE_LOG(LogJoltNetworkPrediction, Display, TEXT("[Interpolate] Interpolation out of sync. We don't have valid frames for this time for this sim. Frame %d. TimeMS: %d. @ %d"), FromFrame, ActualTimeMS, InterpolationTimeMS);
				}

				typename FInstance::FFrame& FrameData = Instance.ClientRecvFrames[FromFrame];
				FJoltNetworkPredictionDriver<ModelDef>::FinalizeFrame(InstanceData.Info.Driver, FrameData.SyncState, FrameData.AuxState);

				InstanceData.Info.View->UpdateView(FromFrame, ActualTimeMS, nullptr, FrameData.SyncState, FrameData.AuxState);
				
				FJoltNetworkPredictionDriver<ModelDef>::DispatchCues(&InstanceData.CueDispatcher.Get(), InstanceData.Info.Driver, FromFrame, ActualTimeMS, 0);

				if (NetworkPredictionCVars::DrawInterpolation())
				{
					FJoltNetworkPredictionDriver<ModelDef>::DrawDebugOutline(InstanceData.Info.Driver, FColor::Yellow, 0.f);
					FJoltNetworkPredictionDriver<ModelDef>::DrawDebugText3D(InstanceData.Info.Driver, *FString::Printf(TEXT("[%d] %d/%d (%d)"), FromFrame, ActualTimeMS, InterpolationTimeMS, (InterpolationTimeMS-ActualTimeMS)), FColor::Black);
				}
			}
		}
	}
...
private:
	...
	struct FInstance
	{
		...
		//
		// It would be nice if we could configure the NetRecv to use an array of TJoltClientRecvData.
		...
		// TJoltClientRecvData also stores more than we would need to here - the InputCmd and ServerFrame aren't
		...
		TJoltNetworkPredictionBuffer<FFrame> ClientRecvFrames;
	};
