...
-They are dispatched *after* the simulation is done ticking (via TJoltNetSimCueDispatcher::DispatchCueRecord which is called during the owning component's tick).
...
Notes on "simulation affecting events" (E.g, *not* JoltNetSimCues)
...
JOLTNETWORKPREDICTION_API DECLARE_LOG_CATEGORY_EXTERN(LogJoltNetworkPredictionCues, Display, All);

#ifndef JOLTNETSIMCUE_TYPEID_TYPE
#define JOLTNETSIMCUE_TYPEID_TYPE uint8;
#endif

using FJoltNetSimCueTypeId = JOLTNETSIMCUE_TYPEID_TYPE;

template<typename ModelDef>
struct FJoltNetworkPredictionDriver;

// ----------------------------------------------------------------------------------------------------------------------------------------------
//	Wrapper: wraps the actual user JoltNetSimCue. We want to avoid virtualizing functions on the actual user types.
// ----------------------------------------------------------------------------------------------------------------------------------------------

struct FJoltNetSimCueWrapperBase
{
	virtual ~FJoltNetSimCueWrapperBase() { }
	...
	virtual EJoltNetSimCueReplicationTarget GetReplicationTarget() const = 0;
};

template<typename TCue>
struct TJoltNetSimCueWrapper : FJoltNetSimCueWrapperBase
{
	TJoltNetSimCueWrapper() = default;

	template <typename... ArgsType>
	TJoltNetSimCueWrapper(ArgsType&&... Args)
		: Instance( MoveTempIfPossible(Forward<ArgsType>(Args))... ) { }

	void NetSerialize(FArchive& Ar) override final
	{
		TJoltNetCueNetSerializeHelper<TCue>::CallNetSerializeOrNot(Instance, Ar);
	}

	bool NetIdentical(const void* OtherCueData) const override final
	{
		return TJoltNetCueNetIdenticalHelper<TCue>::CallNetIdenticalOrNot(Instance, *((const TCue*)OtherCueData));
	}
	...
	EJoltNetSimCueReplicationTarget GetReplicationTarget() const override final
	{
		return TJoltNetSimCueTraits<TCue>::ReplicationTarget;
	}
...
struct FJoltNetSimCueCallbacks
{
...
/** System parameters for JoltNetSimCue events */
struct FJoltNetSimCueSystemParamemters
{
	...
	FJoltNetSimCueCallbacks* Callbacks;	
};
...
public:
	...
	FORCENOINLINE static void RegisterCueType(const FString& TypeName)
	{
		...
		UE_LOG(LogJoltNetworkPredictionCues, Verbose, TEXT("RegisterCueType %s @ 0x%X. (NumPending: %d) "), *TypeName, &TCue::ID, RegisteredTypes.CueTypes.Num());
		...
		{
			&TCue::ID, 
			[](){ return new TJoltNetSimCueWrapper<TCue>();},
			TypeName
		};
		RegisteredTypes.CueTypes.Emplace(TypeInfo);
		
		static_assert(THasMemberFunction_NetSerialize<TCue>::Value || !TJoltNetSimCueTypeRequirements<TCue>::RequiresNetSerialize, "TCue must implement void NetSerialize(FArchive&)");
		static_assert(TJoltHasNetIdenticalHelper<TCue>::Value || !TJoltNetSimCueTypeRequirements<TCue>::RequiresNetIdentical, "TCue must implement bool NetIdentical(const TCue&) const");
	}
	...
	static void UnregisterCueType()
	{
		...
		UE_LOG(LogJoltNetworkPredictionCues, Verbose, TEXT("UnregisterCueType 0x%X."), &TCue::ID);
	...
	void FinalizeCueTypes()
	{
		...
		{
			UE_LOG(LogJoltNetworkPredictionCues, Verbose, TEXT("FinalizeTypes: 0x%X. %d Pending Types"), &RegisteredTypes, RegisteredTypes.CueTypes.Num());
			...
			for (FTypeInfo& TypeInfo: RegisteredTypes.CueTypes)
			{
				...
				*TypeInfo.IDPtr = ++ID;
				...
				UE_LOG(LogJoltNetworkPredictionCues, Verbose, TEXT("    Cue %s assigned ID: %d (0x%X)"), *TypeInfo.TypeName, *TypeInfo.IDPtr, TypeInfo.IDPtr);
			}
	...
	JOLTNETWORKPREDICTION_API static FGlobalCueTypeTable& Get()
	{
		return Singleton;
	}

	FJoltNetSimCueWrapperBase* Allocate(FJoltNetSimCueTypeId ID)
	{
		return TypeInfoMap.FindChecked(ID).AllocateFunc();
	}

	FString GetTypeName(FJoltNetSimCueTypeId ID) const
	{
...
private:
	...
	struct FTypeInfo
	{
		FJoltNetSimCueTypeId* IDPtr = nullptr;
		TFunction<FJoltNetSimCueWrapperBase*()> AllocateFunc;
		FString TypeName;
	};
	...
	JOLTNETWORKPREDICTION_API static FRegisteredCueTypeInfo& GetRegistedTypeInfo();
	
	TMap<FJoltNetSimCueTypeId, FTypeInfo> TypeInfoMap;

	JOLTNETWORKPREDICTION_API static FGlobalCueTypeTable Singleton;
};

// ----------------------------------------------------------------------------------------------------------------------------------------------
// SavedCue: a recorded Invocation of a JoltNetSimCue
// ----------------------------------------------------------------------------------------------------------------------------------------------
...
struct FSavedCue
{
	...
	FSavedCue(const FJoltNetSimCueTypeId& InId, const int32& InFrame, const int32& InTime, const bool& bInAllowRollback, const bool& bInNetConfirmed, const bool& bInResimulates, FJoltNetSimCueWrapperBase* Cue)
	...
	FJoltNetSimCueTypeId ID = 0;
	int32 Frame = INDEX_NONE;
	int32 Time = 0;
	TUniquePtr<FJoltNetSimCueWrapperBase> CueInstance;
	FJoltNetSimCueCallbacks Callbacks;

	bool bDispatched = false;		// Cue has been dispatched to the local handler. Never dispatch twice.
	bool bAllowRollback = false;	// Cue supports rolling back. ie., we should pass the user valid FJoltNetSimCueCallbacks rollback callback (note this is "this saved cue" specifically, not "this cue type". E.g, on authority, bAllowRollback is always false)
	...
	EJoltNetSimCueReplicationTarget ReplicationTarget;
};
...
public:
	...
	void RegisterType()
	{
		...
		CueTypeInfo.Dispatch = [](FJoltNetSimCueWrapperBase* Cue, TCueHandler& Handler, const FJoltNetSimCueSystemParamemters& SystemParameters)
		{
	...
	void Dispatch(FSavedCue& SavedCue, TCueHandler& Handler, const int32& TimeSinceInvocation)
	{
		...
		else
		{
			...
			UE_LOG(LogJoltNetworkPredictionCues, Warning, TEXT("Could not Find NetCue %s (ID: %d) in dispatch table (%d entries)."), *FGlobalCueTypeTable::Get().GetTypeName(SavedCue.ID), SavedCue.ID, CueTypeInfoMap.Num());
			for (auto MapIt : CueTypeInfoMap)
			{
				UE_LOG(LogJoltNetworkPredictionCues, Warning, TEXT("  Have: %d"), MapIt.Key);
			}
		}
	}
...
private:
	...
	struct FCueTypeInfo
	{
		TFunction<void(FJoltNetSimCueWrapperBase* Cue, TCueHandler& Handler, const FJoltNetSimCueSystemParamemters& SystemParameters)> Dispatch;
	};

	TMap<FJoltNetSimCueTypeId, FCueTypeInfo> CueTypeInfoMap;
};
...
// Traits for TJoltNetSimCueDispatcher
struct JOLTNETWORKPREDICTION_API FCueDispatcherTraitsBase
{
	// Window for replicating a JoltNetSimCue. That is, after a cue is invoked, it has ReplicationWindow time before it will be pruned.
...
{
	virtual ~FJoltNetSimCueDispatcher() = default;
	...
	void Invoke(ArgsType&&... Args)
	{
		...
		{
			if (EnumHasAnyFlags(TJoltNetSimCueTraits<T>::SimTickMask(), Context.TickContext))
			{
				constexpr bool bSupportsResimulate = TJoltNetSimCueTraits<T>::Resimulate;
				...
				if (Context.TickContext == EJoltSimulationTickContext::Authority)
				{
					...
					bTransient = TJoltNetSimCueTraits<T>::ReplicationTarget == EJoltNetSimCueReplicationTarget::None;
					bNetConfirmed = true;
				}
				else
				{
					...
					bAllowRollback = (TJoltNetSimCueTraits<T>::ReplicationTarget == EJoltNetSimCueReplicationTarget::All) || bSupportsResimulate;
				...
				if (Context.TickContext == EJoltSimulationTickContext::Resimulate)
				{
					...
					// (not as nice as the non resimulate path, but better than allocating a new FSavedCue+TJoltNetSimCueWrapper and then removing them)
					...
					for (FSavedCue& ExistingCue : SavedCues)
					{
						...
						if (RollbackFrame <= ExistingCue.Frame && ExistingCue.NetIdentical(NewCue))
						{
							...
							UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Resimulated Cue %s matched existing cue. Suppressing Invocation. Time: %d"), *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), Context.CurrentSimTime);
							return;
						}
					}

					
					auto& SavedCue = GetBuffer(bTransient).Emplace_GetRef(T::ID, Context.Frame, Context.CurrentSimTime, bAllowRollback, bNetConfirmed, bSupportsResimulate, new TJoltNetSimCueWrapper<T>(MoveTempIfPossible(NewCue)));
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Invoking Cue %s during resimulate. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d"), *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}
				else
				{
					// Not resimulate case is simple: construct the new cue emplace in the appropriate list	
					auto& SavedCue = GetBuffer(bTransient).Emplace_GetRef(T::ID, Context.Frame, Context.CurrentSimTime, bAllowRollback, bNetConfirmed, bSupportsResimulate, new TJoltNetSimCueWrapper<T>(Forward<ArgsType>(Args)...));
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Invoking Cue %s. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d"), *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}				
			}
			else
			{
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s .Suppressing Cue Invocation %s. Mask: %d. TickContext: %d. Time: %d"), *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), int(TJoltNetSimCueTraits<T>::SimTickMask()), (int32)Context.TickContext, Context.CurrentSimTime);
			}
		}
	}
...
protected:
	...
	{
		return jnpEnsure(Context.CurrentSimTime > 0 && Context.TickContext != EJoltSimulationTickContext::None);
	}
	...
	struct FContext
	{
		...
		EJoltSimulationTickContext TickContext;
	};
...
struct TJoltNetSimCueDispatcher : public FJoltNetSimCueDispatcher
{
	...
	void NetSendSavedCues(FArchive& Ar, EJoltNetSimCueReplicationTarget ReplicationMask, bool bSerializeFrameNumber)
	{
		...
		FJoltNetSimCueTypeId NumCues = 0;
	...
	void NetRecvSavedCues(FArchive& Ar, const bool bSerializeFrameNumber, const int32 InLastRecvFrame, const int32 InLastRecvTime)
	{
		...
		for (int32 CueIdx=0; CueIdx < NumCues; ++CueIdx)
		{
			...
			{
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Discarding replicated JoltNetSimCue %s that is not intended for us. CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			if (SerializedCue.Frame != INDEX_NONE)
			{
				...
				{
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Discarding replicated JoltNetSimCue %s because it is older (%d) than confirmed frame (%d). CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
					continue;
				}
			}
			else if (SerializedCue.Time <= LastRecvMS)
			{
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Discarding replicated JoltNetSimCue %s because it is older (%d) than confirmed TimeMS (%d). CueMask: %d. Our Mask: %d"), *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			for (int32 ExistingIdx=0; ExistingIdx < StartingNum; ++ExistingIdx)
			{
				...
				if (SerializedCue.NetIdentical(ExistingCue))
				{
					...
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Discarding replicated JoltNetSimCue %s because we've already processed it. (Matched %s)"), *GetDebugName(), *SerializedCue.GetDebugName(), *ExistingCue.GetDebugName());
					bUniqueCue = false;
			...
			if (bUniqueCue)
			{
				...
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Received !NetIdentical Cue: %s. (Num replicated cue sent this bunch: %d. LastRecvFrame: %d. Our Mask: %d)."), *GetDebugName(), *SerializedCue.GetDebugName(), NumCues, LastRecvFrame, int(RecvReplicationMask));
			}
		}
	...
	void DispatchCueRecord(T& Handler, const int32 SimFrame, const int32 CurrentSimTime, const int32 FixedTimeStepMS)
	{
		...
		if (LastRecvFrame != INDEX_NONE)
		{
			...
			for (auto It = SavedCues.CreateIterator(); It; ++It)
			{
				...
				{
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Calling OnRollback for SavedCue JoltNetSimCue %s. Cue has not been matched but it <= LastRecvFrame %d."), *GetDebugName(), *SavedCue.GetDebugName(), LastRecvFrame);
		...
		if (RollbackFrame >= 0)
		{
			for (auto It = SavedCues.CreateIterator(); It; ++It)
			{
				...
				if (SavedCue.bPendingResimulateRollback)
				{
					...
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Calling OnRollback for SavedCue JoltNetSimCue %s. Cue was not matched during a resimulate."), *GetDebugName(), *SavedCue.GetDebugName());
					SavedCue.Callbacks.OnRollback.Broadcast();
					SavedCues.RemoveAt(It.GetIndex(), EAllowShrinking::No);
				}
			}
		...
		for (int32 SavedCueIdx = 0; SavedCueIdx < SavedCues.Num(); ++ SavedCueIdx)
		{
			...
			if (!SavedCue.bDispatched)
			{
				...
				if (bDispatchedCue)
				{
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s Dispatching JoltNetSimCue %s {Frame: %d, Time: %d}"), *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time);
					SavedCue.bDispatched = true;
					TCueDispatchTable<T>::Get().Dispatch(SavedCue, Handler, TimeSinceInvocationMS);
				}
				else
				{
					UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s Withholding Cue %s. %d/%d > %d/%d"), *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time, SimFrame, CurrentSimTime);
				}
			}
		}
		...
		{
			UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("Dispatching transient JoltNetSimCue %s."), *TransientCue.GetDebugName());
		...
		if (SavedCuePruneIdx >= 0)
		{
			...
			{
				UE_CLOG(!SavedCues[i].bDispatched, LogJoltNetworkPredictionCues, Warning, TEXT("Non-Dispatched Cue is about to be pruned! %s. SavedCuePruneTimeMS: %d. SavedCuePruneFrame: %d"), *SavedCues[i].GetDebugName(), SavedCuePruneTimeMS, SavedCuePruneFrame);
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Pruning Cue %s. Prune: (%d/%d). Invoked: (%d/%d). Current Time: %d."), *GetDebugName(), *SavedCues[i].GetDebugName(), SavedCuePruneFrame, SavedCuePruneTimeMS,  SavedCues[i].Frame, SavedCues[i].Time, CurrentSimTime);
			}
#endif
	...
	void NotifyRollback(int32 InRollbackFrame)
	{
		...
		for (FSavedCue& SavedCue : SavedCues)
		{
			...
			if (SavedCue.bResimulates && SavedCue.Frame >= InRollbackFrame)
			{
				...
				UE_LOG(LogJoltNetworkPredictionCues, Log, TEXT("%s. Marking %s bPendingResimulateRollback. (RollbackFrame: %d/%d)"), *GetDebugName(), *SavedCue.GetDebugName(), RollbackFrame, InRollbackFrame);
			}
		}
	}

	// Set which cues we should accept locally (if they get sent to us)
	void SetReceiveReplicationTarget(EJoltNetSimCueReplicationTarget InReplicationMask)
	{
	...
	FString GetDebugName() const final override
	{
		...
		if (Driver)
		{
			...
			FJoltNetworkPredictionDriver<ModelDef>::GetDebugString(Driver, Builder);
...
private:
	...
	EJoltNetSimCueReplicationTarget RecvReplicationMask;
...
// Body of JoltNetSimCue header. Gives type a static ID to be identified with
#define JOLTNETSIMCUE_BODY() static FJoltNetSimCueTypeId ID

// ------------------------------------------------------------------------------------------------
// Register a cue type with the global table.
// ------------------------------------------------------------------------------------------------
template<typename TCue>
struct TJoltNetSimCueTypeAutoRegisterHelper
{
	TJoltNetSimCueTypeAutoRegisterHelper(const FString& Name)
	{
		FGlobalCueTypeTable::RegisterCueType<TCue>(Name);
	}

	~TJoltNetSimCueTypeAutoRegisterHelper()
	{
...
#define JOLTNETSIMCUE_REGISTER(X, STR) FJoltNetSimCueTypeId X::ID=0; TJoltNetSimCueTypeAutoRegisterHelper<X> JoltNetSimCueAr_##X(STR);

// ------------------------------------------------------------------------------------------------
// Register a handler's cue types via a static "RegisterJoltNetSimCueTypes" on the handler itself
// ------------------------------------------------------------------------------------------------
template<typename THandler>
struct TJoltNetSimCueHandlerAutoRegisterHelper
{
	TJoltNetSimCueHandlerAutoRegisterHelper()
	{
		Handle = FGlobalCueTypeTable::RegisterDispatchTableCallback([]()
		{
			TCueDispatchTable<THandler>::Get().Reset();
			THandler::RegisterJoltNetSimCueTypes( TCueDispatchTable<THandler>::Get() );
		});
	}
...
#define JOLTNETSIMCUEHANDLER_REGISTER(X) TJoltNetSimCueHandlerAutoRegisterHelper<X> JoltNetSimCueHandlerAr_##X;

// ------------------------------------------------------------------------------------------------
// Register a handler's cue types via an intermediate "set" class with a static "RegisterJoltNetSimCueTypes" function
// ------------------------------------------------------------------------------------------------
template<typename THandler, typename TCueSet>
struct TJoltNetSimCueSetHandlerAutoRegisterHelper
{
	TJoltNetSimCueSetHandlerAutoRegisterHelper()
	{
		Handle = FGlobalCueTypeTable::RegisterDispatchTableCallback([]()
		{
			TCueDispatchTable<THandler>::Get().Reset();
			TCueSet::RegisterJoltNetSimCueTypes( TCueDispatchTable<THandler>::Get() );
		});
	}
...
#define JOLTNETSIMCUESET_REGISTER(THandler, TCueSet) TJoltNetSimCueSetHandlerAutoRegisterHelper<THandler,TCueSet> JoltNetSimCueSetHandlerAr_##THandler_##TCueSet;
