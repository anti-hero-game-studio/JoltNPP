...
// Helper struct to encapsulate optional, delayed writing of new element to TJoltNetworkSimAuxBuffer buffer
template<typename ElementType>
struct TJoltNetSimLazyWriterFunc
{
	template<typename TBuffer>
	TJoltNetSimLazyWriterFunc(TBuffer& Buffer, int32 PendingFrame)
	{
		GetFunc = [&Buffer, PendingFrame]() { return Buffer.WriteAtFrame(PendingFrame); };
	}

	// Hack
	TJoltNetSimLazyWriterFunc(void* Data)
	{
...
// Reference version of LazyWriter. This is what gets passed through chains of ::SimulationTick calls. This is to avoid copying the TFunction in TJoltNetSimLazyWriterFunc around.
template<typename ElementType>
struct TJoltNetSimLazyWriter
{
	template<typename TLazyWriter>
	TJoltNetSimLazyWriter(const TLazyWriter& Parent)
...
template<typename ElementType> 
struct TJoltNetworkSimAuxBuffer
{
	TJoltNetworkSimAuxBuffer(int32 Capacity)
	{
		Init(Capacity);
	}
	...
	TJoltNetSimLazyWriterFunc<ElementType> LazyWriter(int32 Frame)
	{
		return TJoltNetSimLazyWriterFunc<ElementType>(*this, Frame);
	}
...
struct TJoltNetworkPredictionBuffer
{
	TJoltNetworkPredictionBuffer(int32 Capacity=2)
	{
		check(Capacity > 0);
		Init(Capacity);
	}

	TJoltNetworkPredictionBuffer(const TJoltNetworkPredictionBuffer<ElementType, SingleElement>&& Other)
		: IndexMask(Other.IndexMask)
		, Elements(MoveTemp(Other.Elements))
	{
	}

	TJoltNetworkPredictionBuffer<ElementType, SingleElement>& operator=(TJoltNetworkPredictionBuffer<ElementType, SingleElement>&& Other)
	{
		IndexMask = Other.IndexMask;
		Elements = MoveTemp(Other.Elements);
		return *this;
...
template<typename ElementType> 
struct TJoltNetworkPredictionBuffer<ElementType, true>
{
