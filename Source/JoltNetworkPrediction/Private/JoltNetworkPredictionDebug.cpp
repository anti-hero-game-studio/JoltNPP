...
namespace JoltNetworkPredictionDebug
{
	void DrawDebugOutline(FTransform Transform, FBox BoundingBox, FColor DrawColor, float LifetimeSeconds)
	{
		...
		jnpCheck(UJoltNetworkPredictionWorldManager::ActiveInstance != nullptr);
		UWorld* World = UJoltNetworkPredictionWorldManager::ActiveInstance->GetWorld();
...
FAutoConsoleCommandWithWorldAndArgs DrawServerPrimitivesCmd(TEXT("j.np.Debug.DrawServerPrimitives"), TEXT(""),
