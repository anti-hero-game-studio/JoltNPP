...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltNetworkPredictionReplicatedManager)


AJoltNetworkPredictionReplicatedManager::FOnAuthoritySpawn AJoltNetworkPredictionReplicatedManager::OnAuthoritySpawnDelegate;
TWeakObjectPtr<AJoltNetworkPredictionReplicatedManager> AJoltNetworkPredictionReplicatedManager::AuthorityInstance;

AJoltNetworkPredictionReplicatedManager::AJoltNetworkPredictionReplicatedManager()
{
	bReplicates = true;
...
void AJoltNetworkPredictionReplicatedManager::BeginPlay()
{
	Super::BeginPlay();
...
void AJoltNetworkPredictionReplicatedManager::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(AJoltNetworkPredictionReplicatedManager, SharedPackageMap, SharedParams);
}

FDelegateHandle AJoltNetworkPredictionReplicatedManager::OnAuthoritySpawn(const TFunction<void(AJoltNetworkPredictionReplicatedManager*)>& Func)
{
...
uint8 AJoltNetworkPredictionReplicatedManager::GetIDForObject(UObject* Obj) const
{
	// Naive lookup
	for (auto It = SharedPackageMap.Items.CreateConstIterator(); It; ++It)
	{
		const FJoltSharedPackageMapItem& Item = *It;
...
TSoftObjectPtr<UObject> AJoltNetworkPredictionReplicatedManager::GetObjectForID(uint8 ID) const
{
...
uint8 AJoltNetworkPredictionReplicatedManager::AddObjectToSharedPackageMap(TSoftObjectPtr<UObject> SoftPtr)
{
	if (SharedPackageMap.Items.Num()+1 >= TNumericLimits<uint8>::Max())
	{
		UE_LOG(LogTemp, Warning, TEXT("Mock SharedPackageMap has overflowed!"));
		for (FJoltSharedPackageMapItem& Item : SharedPackageMap.Items)
		{
	...
	SharedPackageMap.Items.Add(FJoltSharedPackageMapItem{SoftPtr});
	MARK_PROPERTY_DIRTY_FROM_NAME(AJoltNetworkPredictionReplicatedManager, SharedPackageMap, this);
