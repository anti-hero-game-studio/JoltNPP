...
#include UE_INLINE_GENERATED_CPP_BY_NAME(JoltNetworkPredictionSettings)

#if WITH_EDITOR
void UJoltNetworkPredictionSettingsObject::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (TObjectIterator<UJoltNetworkPredictionWorldManager> It; It; ++It)
	{
...
namespace UE_NETWORK_PREDICTION
{
	...
	FORCENOINLINE void DrawDevHUD(TMap<TWeakObjectPtr<UWorld>, FDevMenuState>& WorldHandlesMap, AHUD* HUD, UCanvas* Canvas)
	{
		...
		if (State.ActiveHUD == INDEX_NONE)
		{
			...
			{
				const FJoltNetworkPredictionDevHUD& DevHUD = State.DevHUDs[idx];
		...
		{
			const FJoltNetworkPredictionDevHUD& DevHUD = State.DevHUDs[State.ActiveHUD];
			for (int32 idx=0; idx < DevHUD.Items.Num(); ++idx)
			{
				const FJoltNetworkPredictionDevHUDItem& HUDItem = DevHUD.Items[idx];
	...
	FORCENOINLINE void DevMenu(const TArray< FString >& Args, UWorld* InWorld)
	{
		...
		{
			const UJoltNetworkPredictionSettingsObject* Settings = GetDefault<UJoltNetworkPredictionSettingsObject>();
			check(Settings);
			State.DevHUDs = Settings->DevHUDs;

			ALevelScriptActor* LevelScript = InWorld->GetLevelScriptActor();
			FJoltNetworkPredictionDevHUD& LevelScriptDevHUD = State.DevHUDs.InsertDefaulted_GetRef(0);
			...
			if (LevelScript)
			{
				for (TFieldIterator<UFunction> FunctionIt(LevelScript->GetClass(), EFieldIteratorFlags::ExcludeSuper); FunctionIt; ++FunctionIt)
				{
					...
					FJoltNetworkPredictionDevHUDItem& Item = LevelScriptDevHUD.Items.AddDefaulted_GetRef();
		...
		else
		{
			if (State.DevHUDs.IsValidIndex(State.ActiveHUD))
			{
				...
				{
					const FJoltNetworkPredictionDevHUDItem& Item = DevHUD.Items[Choice-1];
